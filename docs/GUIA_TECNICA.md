...
**Límite declarado (y cómo se vigila hoy).** Con un único servicio comprometido condicional, `PublicationGatesOrchestrator._proposal_asset_alignment_gate` toma el camino "PASS trivial: sin deuda de entrega" y no reporta la guía como deuda. Gobernar el universo de la matriz —con su efecto sobre el denominador— corresponde a **AC5, dueño C-D** (maestro §4; filas C y D de la matriz del plan). Tras la decisión A4 de FASE-B el comportamiento vigente está assertionado en `test_deuda_ac5_ledger_solo_condicional_pasa_trivial` (aserta `actionable_total == 0` y `coverage_ratio == 1.0`), y ese test **debe ponerse rojo cuando AC5 lo gobierne**: así el límite no depende de que alguien recuerde leer la nota. El caso de servicio contado que sí bloquea lo cubre `test_get_blocking_issues` con `whatsapp_conflict`. Reproducción, opciones descartadas y sus costes medidos: `evidence/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/FASE-B/A4-decision.md`.
...
## Nota Técnica — FASE-G del plan REFACTOR-WHATSAPP-ENTREGA-2026-09-18 (2026-09-20)

**Qué se hizo**: primera sesión de este plan con código de producto. Un verificador AST que descubre la población de callers **sin lista fija de archivos** y exige las señales cuyo default cambia la conducta en silencio (AC7), más el retiro del contrato muerto `whatsapp_validation` (AC16). Sin `v4complete` (contador 0/1), sin red, sin scraping, sin tocar Tribunal, umbrales, flags de bloqueo, `write/publish/suppress` ni `domain_gates.py`.

**Dónde vive la regla**: `scripts/validate_wiring.py` (905 líneas) con la política centralizada en `GOBERNADOS` — cuatro productores: `PainSolutionMapper.detect_pains`, `CoherenceValidator.validate`, `AssessmentBuilder.with_validation` y `V4ProposalGenerator._generate_dynamic_services_table` — y check 11 registrado dentro de `run_all_validations.py --quick`. El artefacto es `.opencode/wiring_report.json`: `politica` (con la **firma real leída del árbol**, no transcrita), `poblacion`, `cobertura`, `excepciones_aplicadas` y `limites`.

**Decisión de diseño que importa a las fases B–F**: gobernar por **tipo del receptor resuelto**, no por nombre de método. El nombre `validate` está en 71 llamadas de clases ajenas (`PrecisionValidator`, `NoDefaultsValidator`, `PlanValidator`, `ContentValidator`, `EnvValidator` y funciones de módulo); gobernarlo por nombre habría hecho el check inusable o lo habrían vaciado a fuerza de excepciones. El resolutor cubre los cuatro casos que AC7 nombra: constructor en línea, asignación/anotación local **incluida dentro de `try`/`if`/`with`**, `self.<attr>`绑定 desde la propia clase y alias de import (`from x import CoherenceValidator as CV`).

**Cómo se midió**: población real del árbol = 611 archivos en alcance, 169 llamadas descubiertas, 70 gobernadas (14 conformes, 3 omisiones con dueño), 74 exclusiones por clase y 25 receptores sin resolver — de los cuales **0 en producción**, que es la cuenta asertada. Par PRE/POST con la **selección literal escrita en el txt**: PRE 1.313 passed / 1 failed / 2 skipped, POST-A idéntico (delta 0), POST-B 1.331 (+18 funciones canónicas nuevas; conteo del repo 4.246 → 4.264). Siete mutaciones del guard real, 7/7 con rojo por el guard y 0 por syntax/import, con el árbol restaurado y verificado por sha256.

**Hallazgos técnicos que cambian un enunciado del plan**:
- La fila F-A' del maestro §1 describe **una** invocación divergente. Medido por AST: son **tres** en `modules/asset_generation/v4_asset_orchestrator.py` — `detect_pains` (286) y dos `CoherenceValidator.validate` (309 y 447) — todas omitiendo `whatsapp_html_detected`. El rojo de AC7 se demostró sobre esa divergencia **antes** de corregirla; corregirla es de FASE-B, y arreglar solo una deja dos rojos y convierte dos excepciones en `EXCEPCION_VAGA`.
- `PAIN_TO_ASSET` de `ConditionalGenerator` **no** tiene clave `no_whatsapp_visible` mientras `pain_ledger.py` sí la mapea a `whatsapp_button`, y su clave `whatsapp_conflict` promete `["whatsapp_button", "whatsapp_conflict_guide"]`. Tres tablas (`PAIN_SOLUTION_MAP`, `PAIN_TO_ASSET`, `ASSET_CATALOG.promised_by`) deciden la misma promesa. **Ninguna es gobernable por kwargs**: es un defecto de contenido, dueño AC2/B. Declarado como límite de esta matriz, no como cobertura.
- El contrato muerto estaba tan muerto que sus dos tests le pasaban `None`; su dato upstream, en cambio, está vivo en ocho líneas de `main.py`. Retirar el parámetro y borrar la variable habrían sido dos cosas distintas.

**Anti-permisividad del registro de excepciones**: tres hallazgos conocidos quedan amparados con `dueno`/`ac`/`baja_cuando`, y una excepción que ya no ampara nada **es** una violación (`EXCEPCION_VAGA`); `--ignore-known` exhibe las cuatro violaciones completas del árbol. Sin esas dos piezas, la única salida de un hallazgo molesto habría sido anadir una excepción y olvidarla — que es exactamente cómo un allowlist se llama a sí mismo «excepción tipada». Nace `L-ENT.13`.

**Límites, dichos en el propio artefacto**: el AST no ve dispatch dinámico ni metaprogramación; un ✅ prueba que la población descubierta está conforme o registrada, **no** que no existan callers invisibles. El check vive en el quick y **no** en el hook de pre-commit —que además no invoca `run_all_validations.py`— porque tocar hooks está prohibido en fases intermedias. `site_presence_report` en `validate` queda como deuda de política con dueño en C/V-1, no como gobernar por gustarse. Y el verificador tuvo su propio hueco de cobertura (un caller productivo dentro de `try` sin resolver, **con el check en verde**): reconstruido el «antes», 8 receptores productivos sin resolver → 0. Nace `L-ENT.12`.

**Backwards compatibility**: sí, con una excepción tipada y visible. `AssessmentBuilder.with_validation` perdió un parámetro que **no tenía default**, así que cualquier caller externo que lo pase recibirá `TypeError` — es el comportamiento deseado (AC16) y el quick lo detecta estáticamente antes de ejecutar. Ninguna otra firma pública cambió.

**Tests**: 18 funciones canónicas nuevas en `tests/test_validate_wiring.py` (rojos por guard sobre fixtures en `tmp_path`, serialización leyendo el JSON del writer real, tres estados del lector, códigos de salida del CLI y no-reescritura del árbol). Quick 11/11; `validate_document_integration.py` en paz; R2 **FUERA DE SERVICIO (R2.1)** con auto-reporte en unidad contable separada.

## Nota Técnica — FASE-0 del plan REFACTOR-WHATSAPP-ENTREGA-2026-09-18 (2026-09-20)

**Qué se hizo**: serializar la evidencia del veredicto para que exista un ZIP entregable, sin tocar la regla que decide. Tres cambios aditivos: el gate anota su recall **fundado**, el acta publica **causas** y la rama de **publicación** registra el hash del paquete entregado (AC20; AC12 en su rama publish). Sin `v4complete` (contador 0/1), sin red, sin scraping. Congelados y verificados por `git diff --numstat`: `TribunalJudge._compute_verdict`, la tabla de cláusulas, `BLOCKING_VERDICTS`, `GATE_BLOCKING_ENABLED`, los umbrales y `DeliveryPackager.write/publish/suppress`.

**Dónde vive la regla**: `PublicationGatesOrchestrator._critical_recall_details` (productor), `ReviewerReport.to_dict` con `ACTA_FINDING_KEYS` / `ACTA_FINDING_TEXT_LIMIT = 240` / `ACTA_FINDING_CAP = 20` (serie del DTO) y `main._record_published_package_evidence`, llamado tras cada `packager.publish(` en las dos ramas de publicación. El writer del reporte **no** cambió: `main._build_gate_report_payload` ya proyectaba `"details": r.details` sin filtrar, verificado leyendo la fuente de la función.

**Decisión de diseño que importa a C, D, E y VERIFY**: la corrección es en el **productor del dato**, no en el detector. `DiagnosisReviewer._check_vacuous_recall` sigue intacto y sigue cumpliendo su contrato — un `details` sin conteo es efectivamente inequívoco —, así que el caso genuinamente vacuo de SR-H2/L-SR5 conserva su defensa. Un `1.0` pasa a ser autoportante: `all_critical_issues_detected` cuando la lista registrada no está vacía y el audit no la contradice, `evident_critical_issues_missed` (con `evident_critical_missed`) cuando el audit evidencia críticos que la lista no cubre, `audit_present_no_critical_issues` en el camino derivado heredado. Cualquier otro caso viaja **sin** anotación a propósito.

**Cómo se midió**: el camino real del `1.0` se identificó **por símbolo y no por línea** (R2.2) y se reprodujo en memoria con el `AssessmentBuilder` real sobre el `audit_report_20260919_150111.json` archivado: el gate reconstruido devuelve `value: 1.0, details: {}`, idéntico al `gate_report` de la corrida (`coincide_con_el_archivo: true`). Contrafactual con `TribunalJudge._compute_verdict` sobre **copia temporal** del acta archivada (`output/` es lectura, contrato de ejecución): `BLOQUEADO` → `APROBADO-CONDICIONAL-PENDING-ONBOARDING`, que no está en `BLOCKING_VERDICTS`. El mutante engañoso quedó registrado: cero del `critical_count` **sin** recalcular la recomendación sigue `BLOQUEADO` porque `ReviewerReport.verified_block` decide por la recomendación. Par PRE/POST con la misma selección literal: 189 → 189 (**delta 0**) y 210 con las 21 pruebas nuevas; la suite nueva contra `git archive HEAD` rompe 16 (13 por aserción, 3 por `ImportError` del helper nuevo, declarados). Regresión completa 3 failed / 4.247 passed / 41 skipped / 4 xfailed, con los tres rojos preexistentes y con dueño. 6/6 mutaciones con rojo causado por el guard, incluidas las dos **prohibidas** por el plan (M5 relajar el detector, M6 sustituir en vez de sumar), restauradas y verificadas por sha256.

**Crecimiento del acta, medido y no estimado**: JSON 1.362 → 2.513 bytes (**+1.151, +84 %**) y **MD +0 bytes** — `ActaWriter._render_reviewer_reports` renderiza una tabla y no consume `findings` —, contra 9.155 bytes de los cuatro `revision_*.json` de esa corrida. La proyección es lista blanca de cuatro claves con texto acotado: un resumen de la causa, no una segunda copia del informe del Bot.

**Hallazgos técnicos que cambian un enunciado del plan**:
- El conteo que publicará `details` en una corrida de hoy es **4**, no 3: `AssessmentBuilder.with_geo_flow` (FASE-G) anexa el issue de banda GEO antes de correr los gates. La cifra del audit son 3.
- `AssessmentBuilder.with_audit` asigna la **misma lista** de `audit_result.critical_issues` y `with_geo_flow` hace `.append` sobre ella: el geo-flow muta el objeto del auditor. Se registra como **DEUDA-0.1 con dueño E** (copia defensiva en el adaptador); FASE-0 no lo gobierna porque es un cambio de adaptación, no de serialización de evidencia.
- Un test asertaba la forma exacta del bloque `reviewer_reports` (`set(entry) == {6 claves}`): cualquier clave nueva del acta habría quedado roja **por re-numeración y no por contrato** — el mismo mecanismo que en G dejó rojo el contract test del `[10/10]` (`L-V2.3`). Se re-ata a superconjunto heredado + subconjunto declarado + coherencia interna (`len(findings) <= findings_count`), sin debilitar la aserción de las cuatro entradas.
- El fixture `audit_with_founded_recall` emparejaba `critical_issues_count: 3` con la base del camino de **cero** críticos. Pasaba porque el revisor solo mira la presencia de la clave. Se corrigió el fixture; el revisor no se tocó.

**Límites, dichos en el artefacto**: P6.2 y P6.5 siguen `NOT_EVALUABLE` por diseño y `T1_CERTIFIABLE_CLAUSES` son 4 — el acta no puede afirmar seis cláusulas certificadas. El tope de 20 hallazgos por revisor es un techo declarado con contador de omisos, no una garantía de exhaustividad. El hash del publicado depende de que el ZIP exista al anotar (si no, viaja `sha256: null` con su `error`). Nada de esto certifica la rama E2E: AC20 está **VERIFICADO OFFLINE** y VERIFY/E2E deben confirmarlo sobre la corrida única; el techo real con tier B sigue siendo `APROBADO-CONDICIONAL-PENDING-ONBOARDING`.
