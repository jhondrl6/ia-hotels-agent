...
class OpenGraphGenerator:
    ...
    def generate_content(self, hotel_data: Dict[str, Any], existing_og_tags: Optional[List[str]] = None) -> str:
        """
        ...
        Args:
            hotel_data: Dictionary containing hotel information
            existing_og_tags: List of OG property names already present on the site
                             (e.g., ["og:locale", "og:type", "og:title"]).
                             When provided, these tags will NOT be duplicated in the output.
                             Default None = generate all tags from scratch (backward compatible).

        Returns:
            HTML string with Open Graph meta tags
        """
        og_data = self._extract_og_data(hotel_data)
        return self._generate_html(og_data, existing_og_tags=existing_og_tags)
    ...
    def _generate_html(self, og_data: HotelOGData, existing_og_tags: Optional[List[str]] = None) -> str:
        """
        Generate HTML with Open Graph meta tags.

        Args:
            og_data: Open Graph data
            existing_og_tags: List of OG property names already present on site.
                             Tags matching these properties will NOT be generated.
                             None = generate all tags (backward compatible).

        Returns:
            HTML string with meta tags
        """
        existing = set(existing_og_tags or [])
        is_enhance_mode = len(existing) > 0
        ...
        # Start building HTML
        mode_comment = "enhance_existing" if is_enhance_mode else "full_generation"
        lines = [
            f"<!-- Open Graph Meta Tags for {og_data.hotel_name} -->",
            f"<!-- Generated by IA Hoteles Agent - {mode_comment} mode -->",
        ]

        if is_enhance_mode:
            lines.append(f"<!-- {len(existing)} OG tags already present on site — generating only missing tags -->")
        ...
        # Only generate tags that are NOT already present
        if "og:type" not in existing:
            lines.append('<meta property="og:type" content="hotel" />')
        if "og:title" not in existing:
            lines.append(f'<meta property="og:title" content="{og_data.hotel_name}" />')
        if "og:description" not in existing:
            lines.append(f'<meta property="og:description" content="{description}" />')
        if "og:url" not in existing:
            lines.append(f'<meta property="og:url" content="{og_data.website_url}" />')
        if "og:site_name" not in existing:
            lines.append(f'<meta property="og:site_name" content="{og_data.hotel_name}" />')
        if "og:locale" not in existing:
            lines.append('<meta property="og:locale" content="es_CO" />')

        # Add image if available (only if not already present)
        if og_data.photo_url:
            if "og:image" not in existing:
                lines.append(f'<meta property="og:image" content="{og_data.photo_url}" />')
            if "og:image:alt" not in existing:
        ...
        if "twitter:card" not in existing:
            lines.append('<meta name="twitter:card" content="summary_large_image" />')
        if "twitter:title" not in existing:
            lines.append(f'<meta name="twitter:title" content="{og_data.hotel_name}" />')
        if "twitter:description" not in existing:
            lines.append(f'<meta name="twitter:description" content="{description}" />')

        if og_data.photo_url and "twitter:image" not in existing:
