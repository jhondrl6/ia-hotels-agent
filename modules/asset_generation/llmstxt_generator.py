...
class LLMSTXTGenerator:
    ...
    def generate(
        ...
        # Warn on missing real data fields
        if not region:
            logger.warning(
                "[LLMSTXTGenerator] 'region' not found in hotel_data. "
                "Geographic context will be limited. Ensure audit_data/hotel_data includes region."
            )
        if not city:
            logger.warning(
                "[LLMSTXTGenerator] 'city' not found in hotel_data. "
                "Location context will be limited."
            )

        # USP: use real data from hotel_data, NOT generic fallback
        usp = hotel_data.get("usp", "")
        if not usp and description:
            usp = description
        if not usp:
            logger.warning(
                "[LLMSTXTGenerator] Neither 'usp' nor 'description' found in hotel_data. "
                "Quote section will use PENDING marker."
            )
            usp = "[PENDING_ONBOARDING: usp/description]"
...
"""
        ...
        # Geographic Context: use real data from hotel_data, NOT hardcoded content
        geo_section = "## Geographic Context\n\n"
        ...
        if len(location_parts) > 1:
            geo_section += f"- **Location:** {', '.join(location_parts)}\n"

        attractions = hotel_data.get("nearby_attractions", [])
        if attractions:
            geo_section += f"- **Tourist attractions nearby:** {', '.join(attractions[:5])}\n"
        else:
            logger.debug(
                "[LLMSTXTGenerator] 'nearby_attractions' not found in hotel_data. "
                "Skipping attractions in Geographic Context."
            )

        coffee_region = hotel_data.get("coffee_region", "")
        if coffee_region:
            geo_section += f"- **Coffee region:** {coffee_region}\n"

        climate = hotel_data.get("climate", "")
        if climate:
            geo_section += f"- **Climate:** {climate}\n"
...
"""
    ...
    def generate_from_assessment(
        ...
        hotel_data = {
            ...
            # Pass through real data fields from audit_data/hotel_data
            "region": assessment_data.get("region", ""),
            "city": assessment_data.get("city", assessment_data.get("addressLocality", "")),
            "usp": assessment_data.get("usp", ""),
            "nearby_attractions": assessment_data.get("nearby_attractions", []),
            "coffee_region": assessment_data.get("coffee_region", ""),
            "climate": assessment_data.get("climate", ""),
        }
