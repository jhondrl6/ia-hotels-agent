...
class FinancialFactors:
    ...
    a config/financial_defaults.yaml.
    ...
    # Hardcoded fallbacks cuando financial_defaults.yaml no está disponible
    _FALLBACK_DEFAULTS = {
    ...
    _FALLBACK_SUPERPOSITION = 0.7

    @staticmethod
    def _load_defaults() -> dict:
        """Load financial_defaults.yaml. Falls back to _FALLBACK_DEFAULTS."""
        try:
            config = load_yaml_config('financial_defaults')
            # Flatten nested comision_ota into flat keys for DEFAULTS compatibility
            defaults = {}
            for key, value in config.items():
                if key == 'version' or key == 'description':
                    continue
                if isinstance(value, dict):
                    # Flatten comision_ota: {min, base, max} → comision_ota_min, etc.
                    for subkey, subvalue in value.items():
                        defaults[f"{key}_{subkey}"] = subvalue
                else:
                    defaults[key] = value
            return defaults
        except (YAMLLoadError, KeyError) as e:
            logger.warning(f"financial_defaults.yaml unavailable: {e}")
            return FinancialFactors._FALLBACK_DEFAULTS.copy()

    @staticmethod
    def get_superposition_factor() -> float:
        """Load superposition_factor from config/financial_defaults.yaml."""
        try:
            config = load_yaml_config('financial_defaults')
            return float(config.get('superposition_factor', 0.7))
        except (YAMLLoadError, KeyError, TypeError, ValueError):
            return FinancialFactors._FALLBACK_SUPERPOSITION
    ...
    def get_config(self, region: str) -> FinancialFactorsConfig:
        ...
        return FinancialFactorsConfig(
            factor_captura_aila=regional_data.get(
                'factor_captura_aila', 
                defaults['factor_captura_aila']
            ),
            comision_ota_min=thresholds.get(
                'comision_ota_min', 
                defaults['comision_ota_min']
            ),
            comision_ota_base=thresholds.get(
                'comision_ota_base', 
                defaults['comision_ota_base']
            ),
            ...
            exclusion_rating_bajo=thresholds.get(
                ...
                defaults['exclusion_rating_bajo']
            ),
            factor_perdida_base=regional_data.get(
                'factor_perdida_base', 
                defaults['factor_perdida_base']
            ),
            factor_perdida_min=regional_data.get(
                'factor_perdida_min', 
                defaults['factor_perdida_min']
            ),
            factor_perdida_max=regional_data.get(
                'factor_perdida_max', 
                defaults['factor_perdida_max']
            ),
