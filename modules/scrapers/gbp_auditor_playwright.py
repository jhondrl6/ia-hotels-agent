"""
GBP AUDITOR - Versión Playwright
================================
Auditoría de Google Business Profile usando Playwright para mayor estabilidad.
Mantiene 100% compatibilidad con la interfaz de GBPAuditor (Selenium).

Ventajas de Playwright:
- 2-3x más rápido que Selenium
- Mejor anti-detección
- Auto-wait inteligente
- Manejo de diálogos nativo
"""
...
class GBPAuditorPlaywright:
    """
    AUDITORÍA DE FUGAS DE RESERVAS PARA GOOGLE BUSINESS PROFILE
    Versión Playwright - Mayor estabilidad y velocidad.

    Interfaz compatible con GBPAuditor (Selenium) para permitir fallback transparente.
    """

    PERDIDA_MENSUAL_BASE = 2_800_000
    ...
    def _init_browser(self) -> bool:
        ...
        try:
            ...
            logging.warning(f"Playwright browser no disponible: {exc}")
            self.browser = None
            self.page = None
            return False
    ...
    def _fallback_profile(self, hotel_name: str, location: str) -> dict:
        ...
        profile['issues'].append('GBP audit no ejecutada: Playwright browser no disponible y sin cache previa')
        profile['meta'].update({
    ...
    def _ensure_main_panel(self) -> None:
        try:
            self.page.wait_for_selector("div[role='main']", timeout=12000)
    ...
    def _extract_metrics(self, html_snapshot: str, profile_data: dict) -> None:
        ...
        try:
            rating_candidates = self.page.query_selector_all("[aria-label*='estrellas']")
            rating_candidates += self.page.query_selector_all("xpath=//*[contains(@aria-label, 'estrellas')]")
        except Exception:
            rating_candidates = []
        ...
        try:
            ...
            for sel in photo_selectors:
                photo_candidates += self.page.query_selector_all(sel)

            photo_candidates += self.page.query_selector_all("xpath=//*[contains(@aria-label, 'Fotos')]")
            photo_candidates += self.page.query_selector_all("xpath=//*[contains(@aria-label, 'photos')]")
        except Exception:
            photo_candidates = []
        ...
        if photos_val is not None:
            ...
            try:
                gallery_imgs = self.page.query_selector_all("button[style*='background-image']")
                if len(gallery_imgs) > 0:
                    photos_val = len(gallery_imgs)
                    profile_data['fotos'] = photos_val
                    confidence['photos'] = False
    ...
    def _attempt_photo_modal(self, profile_data: dict) -> None:
        ...
        try:
            photo_trigger = self.page.wait_for_selector(
                "button[aria-label^='Fotos'], a[aria-label^='Fotos']",
                timeout=5000
            )
        except PlaywrightTimeout:
            confidence.setdefault('photos', False)
        ...
        try:
            self.page.wait_for_selector("div[role='dialog']", timeout=6000)
        except PlaywrightTimeout:
            confidence.setdefault('photos', False)
        ...
        try:
            ...
            try:
                close_button = self.page.query_selector("button[aria-label*='cerrar'], button[aria-label*='Cerrar']")
                if close_button:
                    close_button.click()
            except Exception:
                pass
    ...
    def _scrape_maps_profile(
        ...
        self.page.goto(maps_url, wait_until='load', timeout=120000)
        ...
        try:
            ...
            fugas, perdida = self._detectar_fugas(profile_data, self.hotel_data, self.region)
        ...
        try:
            detector = HorariosDetectorPlaywright(self.page)
        ...
        try:
            whatsapp_elements = self.page.query_selector_all("xpath=//*[contains(text(), 'WhatsApp') or contains(@href, 'wa.me') or contains(@href, 'whatsapp')]")
        ...
        try:
            posts_section = self.page.query_selector_all("[aria-label*='Actualizaciones']")
            if posts_section:
        ...
        try:
            qa_section = self.page.query_selector_all("xpath=//*[contains(text(), 'Preguntas y respuestas')]")
            if qa_section:
        ...
        try:
            ...
            profile_data['gbp_activity_score'] = 100
    ...
    def _detectar_motor_reservas_gbp(self, profile_data: dict) -> dict:
        ...
        try:
            enlaces = self.page.query_selector_all("a[href]")
                ...
                try:
                    ...
                    for patron in patrones_motor:
                        if patron in href:
                            ...
                            label = enlace.get_attribute('aria-label') or enlace.text_content() or ''
            ...
            if not resultado['existe']:
                ...
                for patron in patrones_motor[:8]:
                    if patron in html:
                        resultado['existe'] = True
    ...
    def _calcular_activity_score(self, profile_data: dict) -> int:
        ...
        fotos_meta = pesos.get('fotos_meta', 15)
    ...
    def validate_location_only(
        ...
        *,
        ...
        """Perform lightweight geographic validation without launching Playwright."""
    ...
    def check_google_profile(
        ...
        """
        Main entry point - Same interface as Selenium version.

        Returns:
            dict with GBP profile data including:
            - existe: bool
            - score: int (0-100)
            - issues: List[str]
            - reviews: int
            - rating: float
            - fotos: int
            - ... (same fields as Selenium version)
        """
...
if __name__ == "__main__":
    auditor = GBPAuditorPlaywright(headless=False)
