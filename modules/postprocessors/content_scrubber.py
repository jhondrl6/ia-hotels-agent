...
- [PENDING_*] markers detected and block publication
...
class ContentScrubber:
    ...
    def scrub(
        ...
        pending_result = self._fix_pending_markers(content, fixes)
        content = pending_result['fixed_text']
        if pending_result['issues_found']:
    ...
    # ---------------------------------------------------------------
    # RULE 6: Detect [PENDING_*] markers and block publication
    # ---------------------------------------------------------------
    def _fix_pending_markers(
        self,
        content: str,
        fixes: List[str],
    ) -> Dict[str, Any]:
        ...
        if matches:
            fixes.append(f'Bloqueo: Marcadores pending detectados: {matches}')
            return {
                'fixed_text': content,  # No auto-replacement — requires intervention
                'issues_found': [f"Pending marker found: {m}" for m in matches],
                'block_publication': True,
                'severity': 'CRITICAL'
            }
        return {'fixed_text': content, 'issues_found': [], 'block_publication': False, 'severity': None}
...
if __name__ == "__main__":
    ...
    # Test 1: pending marker detected
    result1 = scrubber.scrub("Test [PENDING_ONBOARDING] content")
    print(f"Test 1 - pending marker:")
    print(f"  block_publication={result1.block_publication}")
    print(f"  fix_count={result1.fix_count}")
    print(f"  fixes={result1.fixes_applied}")

    # Test 2: no pending marker
    result2 = scrubber.scrub("Test clean content")
    print(f"Test 2 - clean content:")
    print(f"  block_publication={result2.block_publication}")
    print(f"  fix_count={result2.fix_count}")