...
> **Versión roadmap**: v4.2 (2026-09-02) — reestructurado bajo el principio rector (v4.0), **corregido contra artefactos reales** (v4.1) y **completado con los tramos de FASE T y los agujeros vivos** (v4.2)
...
> **Qué añade la v4.2** (todo verificado contra código vivo y artefactos el 2026-09-02): **(1)** FASE T partida en **tramo offline** (T0/T1/T2/T4 — certificable ya contra los artefactos de SalenteReal) y **tramo externo** (T3/T5/T6 — bloqueado en datos operativos reales del hotel y en credenciales FTP/WP + staging), con la Definition of Done separada en técnica y comercial porque un Done único no es satisfacible en este repo (§7.2). **(2)** Cuatro agujeros vivos registrados como deuda: **P12** nuevo (el check de coherencia de mayor peso solo corre pre-gen ⟹ P6.3 sin verificación de score en la entrega), **P10** extendido (skip silencioso en el builder de matriz — trampa de falso negativo para T0.3), **H7** extendido (el oráculo de presencia no se persiste ⟹ T0.2 no retro-testeable) y **H9** extendido (G9 se salta en verde — latente, no observado). **(3)** La cifra `0.8261` de Castilla Real calificada como evidencia archivada del 2026-05-28 **no re-verificada**; el 0.88 de SalenteReal queda como única cifra vigente.
...
>
> **Nota pre-gen (P6.3):** el check de mayor peso del validador de coherencia — `promised_assets_exist`, peso **2.0 sobre 7.5** — **solo se ejecuta cuando no hay assets generados** (`coherence_validator.py:670` `if not generated_assets:`, con comentario H6 FIX explícito: *"With real generated_assets, we trust the orchestrator's actual output"*). En la rama de éxito devuelve `score = 1.0` **hardcoded**; el tamaño del registro solo entra al texto del mensaje. Post-generación — que es cuando se decide la publicación — el check confía en el orquestador sin re-verificar el contrato ⟹ **P6.3 no tiene verificación de score en el momento de la entrega**. Este es el mecanismo del Δcoherence = 0.0000 medido en T0.3. Deuda **P12**; condiciona lo que T2 puede certificar.
...
**Lo que falta para cerrar el primer piso**: la **coherencia** está demostrada en score (SalenteReal **0.88** — verificado 2026-09-02 en `asset_generation_report.json`: `coherence_score_pre/post/final = 0.88` y `coherence_report.overall_score = 0.88`). La **corrección** no: el paquete es Tier B/C, producido sin onboarding.

> **Calificación de la otra cifra citada históricamente:** el `0.8261` del E2E Castilla Real es **evidencia archivada del 2026-05-28, no re-verificada contra artefactos en la revisión v4.1**. No citarla como score vigente ni como segunda fuente del argumento — mismo patrón que produjo el error del 0.9133 (§7.1, nota siguiente). La cifra verificada y suficiente es el 0.88 de SalenteReal.
...
Ancla: P6 ("primer piso obligatorio") y P7. Fuente del plan: `.opencode/context/CONTEXT-BOTS-POTENCIALIZACION-IAH-CLI-2026-09-01.md` §10 — ⚠️ **parcialmente refutado** por la auditoría del 2026-09-02 (14 refutaciones factuales + 10 defectos vivos que el doc no ve). Las correcciones verificadas contra código y artefactos están incorporadas abajo. **Desde el 2026-09-02 ese §10 es un stub supersedido que apunta de vuelta a esta sección** (su §10.2 lista lo refutado, §10.3 preserva el detalle de implementación que este ROADMAP no replica — rutas de módulo, convención de tests y de evidencia — y §10.4 mapea las dos nomenclaturas: Etapa 1-6 ↔ T1-T6, A1-A6 ↔ H9/H7/P12/T0.2/P10/G11, N11 ↔ T0.4/P9). **Esta sección es el plan de registro**; el diseño por bot sigue vivo en §5 del documento de contexto.
...
| **T0.3** | Recién entonces tocar el **registro de servicios** | Unificar 7→8 cuesta **exactamente 0.0000 en coherence** (dos candados estructurales: score `1.0` hardcoded en la rama de éxito — el mecanismo es que el check más pesado solo corre pre-gen, deuda **P12**; y denominador por unión en la de fallo) pero **empeora alignment 0.571 → 0.500**. Hacerlo antes de T0.1 es pagar el costo sin recibir el beneficio. ⚠️ **Trampa de falso negativo:** `ProposalAssetMatrix.build` hace **skip silencioso** (`proposal_asset_alignment.py:610-612`, `if not expected_asset: continue`), así que agregar el 8º servicio al catálogo **sin** agregarlo a `PROPOSAL_SERVICE_TO_ASSET` lo desvanece de la matriz sin señal y produce Δ = 0 — la unificación parecería inerte cuando en realidad no se aplicó. Ver deuda **P10** |
...
#### Tramos de FASE T — qué es ejecutable aquí y qué depende de sistemas externos

⚠️ **FASE T no es una fase lineal.** Tres de sus seis sub-fases tienen precondiciones que **ninguna corrida de iah-cli puede producir por sí sola**, porque dependen de datos y sistemas ajenos al repo. Esto estaba implícito en los criterios de aceptación y en ningún lado enunciado; se enuncia ahora para que no se lea como deuda de implementación lo que es falta de acceso.

| Tramo | Sub-fases | Precondición externa | ¿Certificable con artefactos de una corrida Tier B? |
|---|---|---|---|
| **Offline** | **T0, T1, T2, T4** | Ninguna: solo código del repo + artefactos ya generados (`output/FASE-D_salentoreal_post_guard/.../v4_audit/`) | **Sí.** T1 y T2 se certifican retro sobre la entrega real SalenteReal 2026-08-31; T4 con LLM mockeado en tests |
| **Externo** | **T3** | **Datos operativos reales de un hotel** (rooms, occupancy_rate, direct_channel_pct, adr) capturados por onboarding | **No.** Ninguna corrida sobre una URL pública produce `evidence_tier: A`; sin él no hay assets ≥ 0.8 ni G0 cerrado |
| **Externo** | **T5** | **Credenciales FTP / WP-API reales + un sitio de staging**, y conectores fuera de `dry_run` (deuda **P1**) | **No.** Sin deploy real no existe `site_verification_applied: true`, y G12 tampoco puede pasar |
| **Externo** | **T6** | **T3 y T5 en PASS** (marco AOA: automatizar solo lo aprobado) + lista de URLs propias + margen de costo verificado (§5) | **No.** Es escala sobre los dos anteriores |

Consecuencia de planificación: el tramo offline puede cerrarse y certificarse **ahora**, sin esperar nada externo. El tramo externo no se destraba escribiendo más código de pipeline — se destraba con un hotel real que entregue datos y un sitio donde desplegar.
...
**Definition of Done de FASE T** — partida en dos tramos, porque un Done único no es satisfacible en este repo:

- **DoD-técnico (tramo offline, certificable ahora):** (a) ningún paquete sale sin `acta_revision.md` que certifique P6 + P7; (b) ningún paquete Tier B/C recibe `APROBADO-PARA-ENTREGA`; (d) el tribunal no duplica lógica de gates; (e) costo marginal dentro del margen comercial. *Disparador: tests deterministas verdes corridos retro sobre los artefactos reales de SalenteReal 2026-08-31.*
- **DoD-comercial (tramo externo):** (c) ningún deploy se ejecuta sin veredicto de aprobación. *Requiere T5, que requiere credenciales FTP/WP + staging y cerrar la deuda **P1**. Disparador: un deploy de prueba verificado en producción con `site_verification_applied: true`.*

**No mezclar ambos en un único Done**: mientras (c) dependa de infraestructura del cliente, un Done único vuelve incerificable a toda FASE T, incluido el tramo offline que sí se puede cerrar.
...
|------|----------|----------|
| **G0: Primer piso / entrega confiable** | ¿El pipeline entrega un diagnóstico **correcto y consecuente** para un hotel real? Coherencia interna es **necesaria pero no suficiente**: un paquete puede ser autoconsistente y magníficamente erróneo. | **NO CERRADO** (2026-09-02). Score de coherencia demostrado (SalenteReal **0.88** canónico, verificado 2026-09-02; el `0.8261` de Castilla Real es evidencia archivada del 2026-05-28 **no re-verificada** — §7.1) pero **el veredicto real del paquete es `is_coherent: false`** y el gate lo ignora (§7.1); es Tier B/C, sin onboarding, y salió con ZIP. G0 cierra solo con `evidence_tier: A` + assets ≥ 0.8 + acta `APROBADO-PARA-ENTREGA`, lo que exige el **tramo externo** de FASE T (§7.2). Bloquea la escala de entregas (T6), no la prospección |
...
10. **P10 — Seis registros de identidad de servicios, ninguno canónico.** `service_catalog` 8 (config) · `PROPOSAL_SERVICE_TO_ASSET` 7 (`proposal_asset_alignment.py:22-33`) · `ASSET_CATALOG` 25 · `PAIN_SOLUTION_MAP` 22 mapeables · runtime 4 · contract registry 3 CORE + 3 GEO. `ALL_PROMISED_SERVICES` (`:45`) hace que el de 7 mande sobre el de 8 mientras el comentario de `:35-37` dice "All 8". **Decidir cuál manda es precondición de la propuesta dinámica**, que es la única palanca que cierra a la vez `no_breach = 6/7` e `is_coherent = false` (§7.2 causa raíz). **Modo de fallo concreto (verificado 2026-09-02):** `ProposalAssetMatrix.build` (`:575-659`) hace **skip silencioso** en `:610-612` (`if not expected_asset: continue`) ⟹ un servicio presente en el catálogo pero ausente del mapeo **desaparece de la matriz sin dejar rastro**. Requisito: el skip debe producir una entrada con estado explícito (`NO_ASSET_MAPPED`), no una omisión. Hay además **dos builders de matriz**: el anterior y `AssetAlignmentMatrix.build` (`:748-789`), medidos empíricamente **idénticos en 5/5 variantes**, pero el segundo ignora su parámetro `delivery_context` cuando recibe `generated_assets` ⟹ dos rutas de silencio independientes. Duplicación sin divergencia es deuda latente, no seguridad: converger en un solo builder.
...
12. **P12 — El check de coherencia de mayor peso solo corre antes de generar.** `promised_assets_exist` pesa **2.0 de 7.5** — el mayor de los seis — y está acotado por `if not generated_assets:` (`coherence_validator.py:670`), con comentario H6 FIX explícito: *"With real generated_assets, we trust the orchestrator's actual output"*. En la rama de éxito devuelve `score = 1.0` **hardcoded** y el tamaño del registro solo entra al texto del mensaje. Consecuencia verificada: **post-generación, que es cuando se decide la publicación, P6.3 no tiene verificación de score** — y por eso unificar el registro 7→8 cuesta exactamente 0.0000 en coherence (§7.2 T0.3). **No es un bug**: es una decisión (confiar en el orquestador) cuyo alcance no estaba documentado, y es correcta como atajo de costo. Lo que no es aceptable es que el acta se apoye en ella sin declararlo: **T2 (Bots 1 y 3) no puede certificar P6.3 con este check tal como está**. O se re-verifica el contrato post-gen, o el acta declara explícitamente que esa cláusula se certifica por otra vía (presencia del archivo + `asset_path`, que hoy sale `null` — §6.4).
...
7. **H7 — Artefactos timestamped sin índice, y entradas de gate que no se persisten.** `gate_report_<ts>.json` y `financial_scenarios_<ts>.json` no tienen nombre fijo ni manifiesto que resuelva cuál pertenece a la corrida; `pain_ledger.json` no serializa `assets`, así que la matriz no es reproducible desde disco. Todo revisor externo (incluido el tribunal) necesita un paso de resolución de nombres antes de leer. Bloquea la regla "el tribunal lee outputs" (§7.2). **A la misma clase pertenece el oráculo de presencia, que no se persiste en absoluto:** `site_presence_snapshot` se construye en `main.py:2490-2500` y se propaga a seis consumidores (`:2535`, `:2620`, `:2832`, `:2858`, `:2936`, `:3155`), pero `find output -iname "*site_presence*"` da **0 resultados en todo el histórico**. Es la entrada que decide `present_in_production` y por tanto `no_breach`, `unresolved`, `coverage_ratio` y G9 ⟹ **el número más decisivo del gate de alignment no es auditable post-hoc**: el Bot 3 no puede revisar lo que no existe y ninguna corrida pasada puede re-evaluarse bajo un oráculo distinto (para medir hubo que reconstruirlo a mano). Esto vuelve además **T0.2 no retro-testeable** — arreglar el doble oráculo no se puede verificar contra evidencia histórica hasta que el snapshot canónico se escriba junto a los demás artefactos de `v4_audit/`. El concepto ya existe (DT4-R2); falta solo el writer.
...
9. **H9 — Tres rutas de bloqueo del ZIP, un kill switch sin auditar y un gate que se salta en verde.** `main.py:3194` (FAIL o `_claim_escalated`), `:3205`, `:3274`, más `GATE_BLOCKING_ENABLED` (`:2990-2992`, default `true`). Converger en un único punto de decisión antes de que T1 añada el suyo; auditar el kill switch, que hoy permite desactivar todo el bloqueo por variable de entorno sin dejar rastro. **Cuarta vía, silenciosa y en verde (verificada 2026-09-02):** el gate G9 (`proposal_asset_alignment`) del reporte de delivery **se marca como pasado cuando falta la matriz** — `delivery_quality_report.py:251-255` escribe `{"passed": True, "gate": "G9", "skipped": True, "reason": "proposal_asset_matrix.json not found"}`; el summary (`:310-319`) cuenta `passed_count` sobre `gate_results.values()` ⟹ **un gate no evaluado se cuenta como gate pasado**; y hay un **segundo default independiente** en `:325` para cuando la clave no existe, o sea dos fuentes de verdad para la misma clave. Como `BLOCKING_GATE_NAMES` (`:289`) decide con `not result["passed"]` (`:292`), un gate saltado **nunca bloquea**. **Estado: latente, no observado** — en la corrida SalenteReal la matriz sí existía, así que no disparó; es la ruta por la que un paquete *sin* matriz pasaría el gate de delivery de forma vacuamente verde. Requisito: `skipped` debe ser `NOT_EVALUATED`, visible en el acta y sin contar como `passed`; unificar los dos defaults.
