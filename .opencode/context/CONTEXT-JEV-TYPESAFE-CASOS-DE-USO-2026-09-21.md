...
- SDK admite transporte/cliente HTTP inyectable, útil para probar su código real sin red. **Verificado en ejecución el 2026-09-21** con `httpx2.MockTransport`: una llamada real de `client.system_one` serializó el request a `https://api.typesafe.ai/v1/systemone` con su cabecera `Authorization`, parseó la respuesta y devolvió `model = jev-1.13.0`, sin salir tráfico de la máquina.
- Reintentos medidos en el SDK 0.7.0, no solo documentados: `RetryPolicy(max_retries=2, backoff_initial=0.5, backoff_max=5.0, backoff_jitter=0.25, respect_retry_after=True, timeout=30.0)`, con `timeout` como presupuesto total de la llamada. Un 429 cuesta **3 intentos** por defecto y 1 con `max_retries=0`.
- Clases de error observadas: `TypeSafeAuthenticationError` (401), `TypeSafeRateLimitError` (429), `TypeSafeAPIResponseValidationError` (200 con forma inválida) y `TypeSafeError` (sin API key, fallado antes de tocar red). Un 200 al que le falta `usage` **no** produce `usage = null`: es error de validación, así que el contrato no puede interpretarlo como consumo cero.
- `NoulAnswer` expone únicamente `.noul`; no hay campo de confidence. Las respuestas vienen indexadas por los IDs de pregunta enviados.
...
Medición posterior, el mismo 2026-09-21 y bajo autorización explícita de instalación: `typesafe-sdk==0.7.0` **sí** quedó instalado, pero solo en un entorno aislado (`tmp_test/venv-jev-sdk`), porque su resolución exige pydantic 2.13.5 contra el pin 2.12.5 del producto. La tabla de interfaces, reintentos y clases de error está en `10-analisis-post-implementacion.md` §Sonda de instalación. `decision_client.py`, `triage_lesson_relevance.py` y la evidencia del piloto siguen ausentes; autenticación, cuota y saldo de Jev continúan sin comprobar.
