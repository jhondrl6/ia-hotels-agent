...
Base de la concepción y de la auditoría: HEAD `99a33d8`, medido con `git rev-parse --short HEAD`. Antes del ajuste, `git status --short` mostraba el plan y su contexto sin seguimiento; no se interpreta ese estado posterior como refutación de la limpieza anterior a su creación. El operador pidió primero seguir preparando sin commitear, después autorizó el commit de la preparación (**`2c966f6`**, sobre `99a33d8`, con el par de índices en el mismo commit) y por último su push: `2c966f6..99ac860` está en `origin/master` con paridad `0/0` verificada por `git ls-remote`.
...
| P2 | No existen `scripts/decision_client.py` ni `scripts/triage_lesson_relevance.py` | Existencia comprobada con `Path.exists()`; README del hermano con fases pendientes, 2026-09-21 | **Medición vencida en su mitad, re-medida el 2026-09-24 (bloque C de la orden de calidad): `decision_client.py` sí existe** (la entregó FASE-B del hermano y la remedió su bloque A en `fdd397f`); `triage_lesson_relevance.py` **todavía no**, porque FASE-C del hermano sigue sin mandato. Consecuencia actualizada: **la integración sigue bloqueada, pero por una sola de las dos piezas** — y la que falta no se sustituye escribiéndola desde JEV. La muestra y sus controles locales no necesitan esas implementaciones. Re-medir al abrir B |
...
| P6 | Jev tiene API propia, respuestas tipadas y usage de tokens; Noul no trae confidence separada | Documentación API/modelos/Noul/SDK el 2026-09-21 **y sonda ejecutada en el SDK real con transporte falso**: `NoulAnswer` expone solo `.noul`; `usage` llega con `input_tokens`/`output_tokens`; el modelo devuelto se lee en `resp.model` | Contrato explícito de probabilidad, confianza disponible y abstención; no fabricar campos y no esperar `usage = null` |
| P7 | SDK publicado `typesafe-sdk==0.7.0`, Python >=3.10; modelo versionado `jev-1.13.0` aceptado como `model=` explícito | PyPI y docs el 2026-09-21; **instalado y ejecutado offline** el mismo día (sonda en §Sonda de instalación de `10-analisis`): `system_one` devolvió `model = jev-1.13.0` | El pin es usable; el alias `jev-latest` no lo es y sigue sin fijarse |
| P8 | El SDK **no se instala en el entorno principal**: resuelve `pydantic 2.13.5` / `pydantic-core 2.46.5` y el proyecto pinea `pydantic==2.12.5` en la fila correspondiente de `requirements.txt`; en el venv hay 2.12.3 / 2.41.4 | Instalación de sonda en `tmp_test/venv-jev-sdk` (ignorado por git, 28 MB) el 2026-09-21, con `venv/` verificado intacto después | Motivo medido, no precaución genérica: instalarlo en el principal subiría una dependencia pinneada del producto. B congela su propio lock y no toca `requirements.txt` |
...
| AC8 | Antes de cada intento real comprobar permiso con alcance exacto, hashes, saneamiento y presupuesto finito de llamadas/tokens/USD/tiempo/reintentos. Falta o agotamiento impide la siguiente llamada. El cliente se construye con **`RetryPolicy(max_retries=0)`**: medido el 2026-09-21, el default es `max_retries=2` y un 429 produce **3 intentos** por llamada, cada uno potencialmente facturable | B/C; `FASE-B/budget_tests.txt`, `FASE-C/preflight.json`, `FASE-C/consumo.json`; mutation check del guard real; test que cuenta intentos con un 429 repetido |
| AC9 | SDK real instalado en entorno aislado, con transporte falso y cero red; prueba éxito, IDs incompletos, 401, 422, 429, 5xx y timeout, **asertando la clase de error que el SDK realmente lanza** (`TypeSafeAuthenticationError`, `TypeSafeRateLimitError`, `TypeSafeAPIConnectionError`) y no solo un `Exception`. Un 200 **sin `usage`** no degrada a `usage = null`: lanza `TypeSafeAPIResponseValidationError` (medido), así que el contrato debe tratar la respuesta como fallida, nunca como consumo cero | B; `FASE-B/entorno.json`, `FASE-B/requirements-pilot.txt`, `FASE-B/sdk_contract.txt`, `FASE-B/mutation.json` |
...
- Reintentos y timeout, con valores **medidos en el SDK 0.7.0** (sonda offline del 2026-09-21), no deducidos de la documentación: `RetryPolicy(max_retries=2, backoff_initial=0.5, backoff_max=5.0, backoff_jitter=0.25, respect_retry_after=True, timeout=30.0)`, donde `timeout` es un presupuesto total por llamada —no por intento— y `http_statuses` incluye 408, 429 y 5xx. Un 429 con defaults costó **3 intentos**; con `max_retries=0`, 1. El piloto exige `max_retries=0` y que el runner lleve la cuenta de intentos. La concurrencia inicial es serial y cualquier cambio se pre-registra.
...
|---|---|---|
| `scripts/evaluate_jev_pilot.py` | A: preparar/checkear muestra y calcular métricas deterministas; B: ejecutar por la costura y emitir comparación/decisión | **Entregado en su parte A** (⟦rectificado el 2026-09-24: esta fila decía «NUEVO, no implementado»⟧). Lo que existe son los modos `prepare` y `check`; **`run` y `decide` se niegan explícitamente** mientras no haya costura completa ni autorización, que es exactamente la división A/B de esta fila. Re-medir con `--help` |
...
Commit, push, write-back, archivado, instalación de dependencias e inferencias requieren sus instrucciones de alcance. En la sesión del 2026-09-21 se emitieron y ejecutaron tres: el commit de la preparación (`2c966f6`), la instalación del SDK en **entorno aislado** y el push del rango `2c966f6..99ac860` a `origin/master`. Ninguna de ellas autoriza write-back, archivado, inferencias, instalación en `venv/` ni tocar `requirements.txt`. La muestra necesita un commit aprobado **más adelante**, antes de la primera inferencia.
