...
| Commit, push, write-back y archivado | Cada acción requiere autorización separada. El 2026-09-21 se autorizaron y ejecutaron el commit y el push de la preparación (`origin/master` == `HEAD` == `03b9929`, paridad `0/0`); **write-back y archivado siguen sin autorización**. **Publicado:** la corrección documental del punto 4 (README/`04-contrato`/`09-documentacion`/`10-analisis`, re-anclajes de `venv` + sección §Alineación de pydantic) quedó en `bf24b1b` y **empujada a `origin/master`** (push `03b9929..60cce03`; ver README §Pendientes priorizados, P0). |
...
7. Commit/push se solicitan por separado. **Los cinco cortes se declaran y verifican sin commit** —implementación terminada, verificación terminada, cierre documental, listo para revisión, espera de autorización—, y el `git commit` es una acción posterior, separada y opcional: no es el quinto corte ni condición de ninguno (proceso común del bloque B de `ORDEN-CAMBIO-CALIDAD-PROCESO-2026-09-22.md`, alineado el 2026-09-24). Esto **no** toca la puerta de seguridad propia del piloto: la muestra y su protocolo siguen versionados **antes de la primera inferencia**, que es un requisito de congelación, no un corte de cierre. RELEASE re-lee la interfaz del writer QMind antes de cualquier publicación; write-back antes de archivado y nueva regeneración del índice después, solo con permisos explícitos.
