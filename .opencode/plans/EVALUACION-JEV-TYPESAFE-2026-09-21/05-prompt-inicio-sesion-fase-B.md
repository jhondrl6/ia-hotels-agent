...
**Estado: BLOQUEADA POR DEPENDENCIA TÉCNICA** ⟦**ya no por ésa**: re-medido el 2026-09-25 por la conciliación final de `ORDEN-CAMBIO-CALIDAD-PROCESO-2026-09-22.md`⟧. Requiere A propia y **entrega verificada de B y C offline del hermano**. ⟦Reconciliado el 2026-09-24 por el bloque C de la orden de calidad, fila `JEV`, y **re-visto el 2026-09-25**: de esa puerta, **B del hermano está entregada** (`scripts/decision_client.py`, remediada en `fdd397f`) y **C también lo está** — el piloto FASE-C del hermano se ejecutó y cerró el 2026-09-24 con mandato propio y `scripts/triage_lesson_relevance.py` está versionado en `5817edd`. La frase que aquí decía «C no … sigue sin mandato» quedó vencida por esa ejecución ajena, **no** por una decisión de este plan⟧. Con las dos dependencias entregadas, **ningún bloqueante de esta fase es técnico**: quedan **la revisión humana de la muestra (P1)**, **la decisión del *gap* de interfaz** (maestro §Interfaz) y **el mandato de ejecución de FASE-B, que no se ha dado** — ni esta conciliación lo da ni lo interpreta. Y **se re-mide con `ls` y con la selección de tests del hermano al abrir, no con esta línea**. **`D` y el `RELEASE` del hermano no son precondición de esta fase**: postergar la integración hasta que el hermano cierre es regla de **orden** (gobernanza del índice compartido), no imposibilidad técnica, y esa regla no se rebaja ni se interpreta al revés para adelantar. **AC15 semántico parcial por `NO-EJERCITADO` es admisible como entrada**: no exigir la calidad que el piloto todavía debe medir, y no esperar una aceptabilidad real que la regla de cero red del hermano no puede producir. **Prohibido destapar el bloqueo duplicando la costura, escribiendo el triaje ajeno, o recortando AC1/AC2 del piloto**: si el contrato vigente impide un AC, se deja checkpoint y se pide decisión (maestro §Interfaz).
