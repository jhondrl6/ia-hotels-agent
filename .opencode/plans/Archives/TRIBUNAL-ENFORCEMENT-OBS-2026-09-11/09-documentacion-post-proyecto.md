...
| `dependencias-fases.md` | P1 | Diagrama y tabla reordenados (P3→P2) · **FASE-VERIFY cerrada en no activa** (🔁 reabierta el 2026-09-15 por D-AJUST.4) · prompt de P2 marcado como creado |
...
| `evidence/FASE-P6/` (NR1 `nr1_baseline_pre.txt`/`nr1_baseline_post.txt`/`nr1_delta_r2_7.md`, **5 pares NR7** `nr7_mutation_checks.md`) | P6 | Evidencia de cierre: delta R2.7 (+20, resta verificada contra el PRE propio de P5), 5 mutation checks verde/rojo (uno por AC-G) **[anotado: falso al cierre — ver anotación 6 en §Anotaciones P6-R]**, 0 regresiones |
...
6. Fila §A de evidencia: «**5 pares NR7** … 5 mutation checks verde/rojo (**uno por
   AC-G**)» — **FALSO al cierre** (auditoría I3): los 5 originales cubrían solo AC-G4
   y AC-G5 mutando expectativas, con **cero** para AC-G1/G2/G3. El registro ya está
   anotado en `evidence/FASE-P6/nr7_mutation_checks.md` §ANOTACIÓN P6-R; esta fila se
   deja con el puntero para que nadie la lea aislada. Los 5 pares **por reversión del
   fix** (G1, G2×2, G3, G5) llegan en P6-R; AC-G4 queda sin par por reversión, declarado.

**Estado P6 tras P6-R (para FASE-VERIFY / AC-V0)**: 26 funciones nuevas canónicas
