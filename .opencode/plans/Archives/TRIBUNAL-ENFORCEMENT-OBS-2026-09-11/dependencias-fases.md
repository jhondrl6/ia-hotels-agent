...
> **Regla vigente (D-PRE.1, 2026-09-15; actualizada por FASE-RELEASE)**: P4 permanece cerrada; P5 cerrada; **P6 cerrada + P6-R (remediación post-auditoría el mismo día)**; **FASE-VERIFY cerrada el 2026-09-15** (matriz de 25 ACs + integración cross-fase + greps + triaje); **FASE-RELEASE-4.77.0 cerrada el 2026-09-15 → plan 9/9 COMPLETADO** (P6-R no es fase nueva, es la corrección de P6). RELEASE ejecutó AC-V1 = verificar y citar la matriz de VERIFY (comprobación de completitud en `10-analisis` §Matriz de certificación), bump + sync + CHANGELOG/GUIA + write-back QMind pre-archivado + archivado R2.5 + tag `v4.77.0`. **master y tag `v4.77.0` empujados a origin el 2026-09-15** (L3 de pre-push ofrecido y descartado por el operador; `v4.76.0` verificado en origin y no rehecho). El antiguo escenario de diferimiento de P4 se conserva solo como antecedente.
