...
| FASE-E2E ✅ | T1-T4-B (T1/T2-C/T4-B ⚠️), T2-A/T2-B/T4-A ✅ | VERIFY | Pipeline completo con tribunal integrado + residuos heredados curados. ✅ **Cableado de los 4 revisores en `main.py` ejecutado** (decisión Q1, Vía A; commit `7e1bbc3`): revisores tras el packaging con extractor LLM compartido, 4/4 `revision_*.json` en output real (2026-09-11). Desvío D-T2C-A1 consumido: veracidad «Presente en sitio» VERAZ (3 servicios con `site_verified: true`; `llms_txt` no reclamado). Residuo nuevo: el Juez no consume `reviewer_reports` (diseño T1 — L-E2E.3) y el acta lee `evidence_tier C` vs MANIFEST B (L-E2E.1) |
| FASE-VERIFY ✅ | E2E ✅ | RELEASE | AC1-AC16 certificados contra output E2E real: **15 ✅ + 1 ❌ (AC8**, causa raíz fijada por sonda read-only — 2 capas: deliveries ZIP-only + heurístico stub descalibrado — routed a seguimientos con dueño, D-V.4**)**. ACs propuestos (dueño VERIFY): AC17 ✅ (D-V.2 corrige nota E2E) / AC18 ✅ / AC19 ⚠️. Greps residuales 0 matches en 4/4. Delta verificado (+6 artefactos tribunal, coherence 0.88→0.83 NR5 ✅, `is_coherent` false→true, `no_breach` 6→0). Decisiones D-V.1–D-V.4 (D-V.3 endosa endurecimiento del executor → dueño RELEASE). Cero código de producción tocado. **Desbloquea RELEASE** |
