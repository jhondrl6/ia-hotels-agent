...
| FASE-B | 2 | Biyección mapa↔emisión de `detect_pains` (V1): decidir por cada uno de los 9 pains muertos si se implementa o se retira del mapa + candado de biyección | **MEDIA-ALTA** | **DIRECTO** (decisión de producto por pain: implementar vs retirar cambia lo que se vende) | No |
...
| AC4 | Biyección mapa↔emisión fijada: cada `pain_id` declarado en `PAIN_SOLUTION_MAP` (27 entradas) o tiene punto de emisión real en `detect_pains`, o fue retirado del mapa con justificación registrada. 0 pains muertos sin decisión | B | Test de biyección (patrón guardián AST de FASE-SR-A) | V1, caída #4 de §4 |
