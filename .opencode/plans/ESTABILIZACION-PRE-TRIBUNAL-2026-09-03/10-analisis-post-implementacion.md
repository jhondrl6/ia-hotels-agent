...
**Usadas con solo 3 de 11 fases cerradas**: **≥514 / ≤440** (A 55 + B **345** + D 114), medidas con
el mismo instrumento y el mismo corte (ids de mensaje de asistente únicos **hasta el commit de código**
de cada fase). ⚠️ **El presupuesto total del plan ya está excedido en ~17%** y quedan 8 fases, entre
ellas la de complejidad MÁXIMA (C, presupuestada en 60). Es un **piso**: ninguna de las tres cifras
incluye el cierre documental (el de D añade ~133 y el de B ~14 y sigue creciendo mientras la sesión
paralela trabaje). ⚠️ **No son comparables entre sí**: las de B y D están **medidas** del transcript
de cada sesión; la de A coincide exactamente con su presupuesto, así que parece un tope alcanzado y no
una medición, y **no es reconstruible** con este instrumento. El conteo quedó instrumentado en
`evidence/FASE-D/measure_iterations.py` (re-ejecutable, con corte por timestamp); las mediciones de B
están en `evidence/FASE-B/faseB_iteraciones.txt` y las de D en `evidence/FASE-D/faseD_iteraciones.txt`.
**Esto ya no es un desvío de fase sino un dato del plan**: o los presupuestos por fase están
subdimensionados ~3× de forma sistemática, o la unidad «iteración» del executor no significa lo que
las fases creen. Lo resuelve **VERIFY** → §5 S22 y §7.
...
| S22 | **El presupuesto de iteraciones por fase no era medible ni comparable, y ahora que se mide resulta que está sistemáticamente subdimensionado** — parcialmente cerrado por D | FASE-B (§1 y §7 de este archivo) + FASE-D | B tenía ≤40 iteraciones y usó **345 medidas** al corte de su commit de código `e6d28b8` (**8.6×**); D tenía ≤35 y usó **114** al corte de `76e0257` (**3.3×**). Sumadas a las 55 de A dan **≥514 sobre un presupuesto total de ≤440 para las 11 fases**, con 8 todavía pendientes. Y la cifra de A (55) coincide exactamente con su presupuesto, así que parece un **tope alcanzado** y no una medición: el plan no tenía contador canónico, cada fase reportaba lo que podía, y el total sumaba cosas medidas de forma distinta. El executor R2 fija el tope pero **no el instrumento** | **Parcialmente cerrado**: la sesión de D midió con la **misma unidad** que B (ids de mensaje de asistente únicos del transcript) y dejó el conteo como **artefacto re-ejecutable**, `evidence/FASE-D/measure_iterations.py <transcript> [corte-ISO]`, no como afirmación auto-reportada; las mediciones están en `evidence/FASE-D/faseD_iteraciones.txt` y `evidence/FASE-B/faseB_iteraciones.txt`. Falta: (1) adoptarlo como **instrumento canónico del plan** y correrlo al cierre de cada fase restante, **fijando el corte** — «hasta el commit de código» es el único que hace comparables a B y D, porque el cierre documental añade 14 a B y 133 a D; (2) decidir qué hacer con A, cuya cifra no es reconstruible con ese método; (3) **recalibrar los presupuestos por fase o retirar la métrica**: que las dos fases medidas excedan en 3.3× y 8.6× no es dos accidentes, es un patrón — o el executor cuenta otra cosa que las fases, o los presupuestos se estimaron sin base; (4) **VERIFY** resuelve. No es defecto de código: es defecto del instrumento de seguimiento |
...
| S26 | **NR5 está mal formulada: un conteo literal de tests aprobados no puede ser invariante de un plan que agrega tests** | FASE-D. NR5 dice «baseline **848 passed / 2 skipped** preservado» y se la asigna a **todas** las fases. A la preservó (delta 0), B la preservó (mismo número, medido antes de D), y **D la rompió por construcción**: sus 24 tests nuevos viven en `tests/quality_gates`, así que la corrida pasó a **872 passed** (`evidence/FASE-D/faseD_baseline_{pre,post}.txt`, delta = exactamente +24, 0 failures) | No es un error de D ni del baseline: **la métrica está mal escrita**. Una fase que cumple su plan agregando tests viola la NR; una fase que no toca nada la cumple sin haber hecho nada. Tal como está, las 6 fases que quedan (C, E, F, G, H, I) tienen que registrar una violación verdadera cada una, lo que destruye el señal/ruido de la NR | **VERIFY reformula NR5 como regla de delta**: `passed = baseline + tests nuevos de esta fase`, `skipped idéntico`, **0 fallos ajenos a la fase**, medido sobre `tests/quality_gates` + `tests/asset_generation`. Si se deja el número literal, hay que dejar constancia de que **no es una violación** — y eso es exactamente lo que la NR existe para evitar decir |
...
| Métrica | Valor |
|---------|-------|
| Fases completadas | **3 / 11** — FASE-A ✅, FASE-B ✅ y FASE-D ✅ (las tres 2026-09-03). D se ejecutó en **sesión paralela** a B sobre el mismo working tree, fuera del camino crítico; su código está en `76e0257` → **S21** |
| Iteraciones totales usadas | **≥514 / ≤440** contando solo implementación hasta el commit de código de cada fase (A 55 auto-reportadas + B **345** + D **114**) ⟹ pero **≥661** si se suma el cierre documental, porque D llegó a **247** con su pasada de documentación y B a **359** y sigue en vuelo. **El presupuesto global del plan está agotado y excedido en ~17% con 8 fases pendientes**, entre ellas la de complejidad MÁXIMA (C, presupuestada en 60). Unidad canónica: **ids de mensaje de asistente únicos en el transcript**, contados con `evidence/FASE-D/measure_iterations.py` (artefacto re-ejecutable; mediciones y cortes en `evidence/FASE-D/faseD_iteraciones.txt` y `evidence/FASE-B/faseB_iteraciones.txt`). Por fase: B usó **345** de ≤40 (**8.6×**) y D 114 de ≤35 (**3.3×**) solo en implementación. ⚠️ **Tres advertencias**: (1) la cifra de A no es reconstruible con este método (coincide exactamente con su presupuesto ⟹ parece tope alcanzado, no medición → **S22**); (2) **el «151 de B» que esta fila publicaba estaba corroborado y aun así era falso**: su id único #151 sí aparece en `2026-09-03T21:38:54Z`, pero eso prueba que B llevaba 151 a las 16:38 locales, **no que hubiera terminado** — al corte real (commit `e6d28b8`) son **345**. Corroborar la cita no corroboró la premisa, y además **lo detectó la sesión paralela de D, no B**: fue D la que dejó escrito «la cifra de B al cierre real es mayor que 151». B reprodujo después el corte de las 16:38 (145 ids) y verificó la unidad de conteo (842 registros `assistant` → 358 ids únicos, `isSidechain=false` en todos ⟹ no hay subagentes inflando) → **S22/L-B3**; (3) el instrumento mismo tuvo un defecto que subcontaba el corte a **0** (mezcla de epoch-ms e ISO-8601 en el transcript) → **L-D5** |
| Tests al inicio | 3,689 (`def test_`, medido en `ed8b033`, el commit de concepción del plan) |
| Tests al cierre | **3,763** funciones / **288** archivos `.py` en `tests/`, medido con `grep -rE "^\s*def test_" tests --include=*.py \| wc -l`. **Árbol de trabajo == HEAD `e6d28b8`** (`git grep -h "def test_" HEAD -- tests` = 3.763; `git ls-tree -r --name-only HEAD -- tests \| grep -c '\.py$'` = 288) ⟹ las **tres** fases cerradas del plan están commiteadas y la contaminación ya no es posible: pre-plan **3,689** + A **21** + B **29** + D **24** = **3,763** (+74 atribuibles al plan). ⚠️ `AGENTS.md` sigue declarando **3,689 / 284** en sus **4 sitios** (`README.md` **no** declara conteos — ver **S11**) |
| Contract tests agregados | **74 funciones / 107 casos** — A: 21/37 (`tests/common/test_service_identity_registry.py`, contrato del canónico); B: **29/46** (`test_pain_map_bijection.py` 11/28 + `test_detect_pains_emisiones_faseB.py` 18/18, candado de biyección); D: **24/24** (`test_gate_severity_lists.py` 8 guardianes AST de las listas de severidad + `TestFASEDGateSeverity` 12 conductuales del régimen y el piso + `TestAdvisoryDisclosureFASED` 4 de divulgación en `human_checklist`; sin parametrizar, medido con `pytest --collect-only` = 24 collected). Los de D son **candados de régimen**, no contratos entre módulos: la categoría los agrupa porque el plan mide «cuánta verificación quedó fijada» |
| Versión publicada | 4.74.1 → *(4.75.0 al cierre; ninguna fase intermedia la mueve)* |
| ACs certificados | 0 / 12 formalmente — **6 con evidencia aportada** (AC1, AC2, AC3 por A; **AC4** por B; **AC7, AC8** por D); VERIFY certifica |
| NRs certificados | 0 / 12 formalmente — **NR5 verificado en A y preservado en B** (848 passed / 2 skipped / 11 warnings en `tests/quality_gates` + `tests/asset_generation`; `evidence/FASE-B/faseB_baseline.txt`). B no añadió ni quitó ningún test de esos dos directorios. ⚠️ **El baseline se movió después**: D añadió 24 tests a `tests/quality_gates` ⟹ hoy la misma corrida da **872 passed / 2 skipped / 0 failed** (`evidence/FASE-D/faseD_baseline_{pre,post}.txt`, delta exacto de +24). La evidencia de B es válida porque se midió **antes** de que D aterrizara, y **la NR está mal formulada**: un conteo literal no puede ser invariante de un plan que agrega tests → **S26** / **L-D3** |
| Hallazgos del dossier superados | **6** de (6+4+8/3+16+8) — V2, V3, V14 y P10 por A; **V1** por B; **H10** por D *(Eje 5, fix completo según `06-checklist-implementacion.md`)* |
| Lecciones capitalizadas | **16** (FASE-A: L-A1…L-A6; FASE-B: **L-B1…L-B5**; FASE-D: **L-D1…L-D5**) — VERIFY consolida el total |
| Coherence baseline → final | 0.88 → *(sin cambio: A es delta cero por diseño; B no toca la propuesta ni el validador de coherencia. La primera corrida `v4complete` del plan es FASE-I)* |
