...
| 2 | FASE-B | Biyección **triple** mapa↔emisión↔narrativa (N-A1) | MEDIA-ALTA | DIRECTO | 40 / — | ⬜ Pendiente | N-A1 agrandó el alcance (11 decisiones, 2 archivos fuente + 1 YAML) sin agrandar el presupuesto |
...
| AC4 | Biyección **triple** mapa↔emisión↔narrativa (**11** pains resueltos: 9 muertos + 2 emitidos-y-descartados, N-A1) | ⬜ | *(pendiente)* | *(pendiente)* | B |
...
| §3 biyección | 0 tests fijan la biyección mapa↔emisión↔narrativa (el dossier la planteó doble; N-A1 la hizo triple) | ⬜ | *(pendiente)* | *(pendiente)* |
...
| V3 | ≥9 registros no canónicos | ✅ Superado (FASE-A 2026-09-03) | Censo real: **15** registros (`evidence/FASE-A/censo-registros.md`; 14 al cierre de A + el #15 hallado post-cierre, `regional_benchmarks.yaml::pain_narratives` → C-5/S14, decidido por FASE-B). Arquitectura de dos capas: Capa 1 `PainSolutionMapper.PAIN_SOLUTION_MAP` (27 pains, universo de pain_id, contenido intacto) + Capa 2 `SERVICE_IDENTITIES` (8). **6 derivados**, **4 validados contra Capa 1** con razón registrada, **3 fuera de alcance** (1 canónico + 1 Capa 1 + 6 + 4 + 3 = **15**, cuadra con §8.2 del censo). La perla `monthly_report → no_faq_schema` ya no existe en ningún registro. Lo fijan `test_registro_derivado_no_es_una_copia_literal` (AST, fixpoint transitivo), `test_un_asset_no_se_atribuye_a_pains_distintos_entre_registros` y los `test_*_valida_contra_capa_1` |
...
| S5 | **2 tests rojos preexistentes en `test_proposal_dynamic.py`** — NO son regresión del plan | FASE-A. Verificado de forma reproducible con `evidence/FASE-A/faseA_s5_preexist_verify.py` (salida: `.txt`, `VEREDICTO: S5 confirmado`) contra el baseline pre-cambio `evidence/FASE-A/faseA_predinamico.txt`: **PRE 8 rojos / POST 2 rojos** ⟹ FASE-A cerró 6 fosilizados (L-A5) y los 2 restantes tienen la **aserción idéntica** antes y después. No es «byte-idéntico» literal: difieren direcciones de objeto, duraciones y el número de línea *del propio archivo de tests* (`:345`→`:376`, `:463`→`:494`), que se desplazó porque FASE-A lo editó al invertir esas 6 aserciones | (1) `TestTechnicalAssetsTable::test_technical_assets_table_shows_both_assets` espera «Optimización de Tráfico Indirecto» en `TECHNICAL_ASSET_CATALOG`; (2) `TestFase3ConditionalServicesFiltering::test_no_assets_no_presence_all_excluded` choca con el footnote D-PF1 (territorio de FASE-C). Ambos ya fallaban antes de tocar nada | **No atribuirlos al plan.** FASE-C decide sobre el segundo al reescribir la promesa; el primero es de catálogo técnico, fuera de este plan |
| S6 | **N-A1 — precondición dura de FASE-B**: `narratives` y `detect_pains` son las **dos mitades del mismo agujero** | FASE-A, medido en `evidence/FASE-A/faseA_narratives_audit.txt` | `_pain_to_brecha` descarta en silencio en **`v4_diagnostic_generator.py:3346-3347`** (`if pain.id not in narratives: return None`); el llamador `_identify_brechas:3206-3209` hace `if brecha:` sin loguear ni justificar. `narratives` (`:3263-3344`) tiene **16** claves frente a las **27** de Capa 1 ⟹ **11 ausentes**, que se parten en dos grupos disjuntos: **(a) exactamente los 9 pains muertos de V1** — no llegan porque tampoco se emiten; **(b) 2 que SÍ se emiten y SÍ se descartan hoy**: `no_ga4_enhanced` y `low_ota_divergence`. B1/B2 que solo arregle la emisión deja los 9 rebotando en `:3346`: test de biyección en verde y diagnóstico sin mencionarlos (verificación de premisas fallida, patrón ROADMAP v4.0→v4.1) | **FASE-B debe tratar la biyección como triple** (mapa↔emisión↔narrativa), no doble. Inyectado en `05-prompt-inicio-sesion-fase-B.md`. Ver S12 y S13 para los 2 pains vivos |
...
| S12 | **`no_ga4_enhanced` — novena caída silenciosa VIVA, no catalogada** | FASE-A (N-A1, `evidence/FASE-A/faseA_narratives_audit.txt`) | Se emite hoy en `pain_solution_mapper.py:703-712` (`elif status and hasattr(status, "is_enhanced")` → `Pain(id="no_ga4_enhanced", severity="low", detected_by="analytics", confidence=0.6)`) y `_pain_to_brecha:3346` lo descarta porque no tiene entrada en `narratives`. Es el único de los 11 que está **fuera del dossier**: no aparece en §12.3 (V1-V16), ni en §4 (las 8 caídas), ni en el censo A1. El grep del plan completo da 0 resultados. Su entrada en el mapa (`:188-194`) es priority 3 / impact low / `confidence_required: 0.0` | **FASE-B, grupo (b) de B1**: decidir narrativa-o-retiro. Como es `severity="low"` y `confidence=0.6`, la decisión comercial es si una brecha baja de GA4 avanzado es vendible o ruido. Si se retira del mapa, verificar que no huérfaniza un asset del catálogo |
| S13 | **`low_ota_divergence` — orden forzoso nuevo B→H** | FASE-A (N-A1) + dossier §12.3 V7 | Triple defecto de V7: el guard `hasattr(direct_field.value, '__iter__')` en `pain_solution_mapper.py:453` hace el pain no-disparable con valor numérico, así que hoy **nunca se emite**. Pero tampoco tiene entrada en `narratives`. Si FASE-H arregla V7 sin que B le haya dado narrativa, el pain pasa de **«nunca dispara»** a **«dispara y se desvanece»** — peor para auditabilidad, porque el test de V7 pasa en verde y la caída se vuelve invisible en vez de inexistente | **B antes que H**, forzado. FASE-B le da entrada en `narratives` (o lo retira del mapa, y entonces V7 pierde sentido y H debe saberlo). Inyectado en `05-prompt-inicio-sesion-fase-B.md` §Restricciones y en `dependencias-fases.md` §0. FASE-H debe verificar que B lo resolvió antes de tocar el guard |
| S14 | **Registro #15 que el censo de FASE-A no contó**: `config/regional_benchmarks.yaml::pain_narratives` | FASE-A post-cierre (N-A1), medido en `evidence/FASE-A/faseA_yaml_narratives_audit.txt` y `faseA_yaml_region_blind.txt`. Registrado como **C-5** en `evidence/FASE-A/censo-registros.md` §8.1 | Son **4 copias literales** (una por región: `eje_cafetero`, `caribe`, `bogota`, `antioquia`; sin anclajes YAML) de las **mismas 16** claves pain_id→impacto, **más 16 fallbacks hardcodeados** en el dict `narratives` de `_pain_to_brecha` (`pain_narratives.get(k, 0.20)`): **80 literales para 16 valores**, y las 4 regiones son **idénticas entre sí** (0 divergencias medidas) ⟹ la dimensión regional es vacía. 0 huérfanos vs Capa 1 y **los mismos 11 ausentes**. El YAML se autodescribe como *«CRÍTICO: Estos valores determinan las 4 Razones con impacto monetario»* y cita `v4_diagnostic_generator.py L2240-2311`, que es **fósil**: el dict real está en `:3263-3344`. No se tocó en A: es dinero-adyacente y el dueño de la decisión es quien reescriba `narratives` | **FASE-B** lo absorbe en la decisión L-NC4 de B2: si se rellena `narratives` literal a 27, hay que añadir 11 valores × 4 regiones + 11 fallbacks Python, o los pains nuevos heredan un impacto por defecto sin que nadie lo note (familia V6/P11/S7). Camino preferido: derivar o colapsar las 4 copias en una. Si B no lo hace, pasa a **FASE-RELEASE** como deuda documentada |
| S15 | **Las citas de línea del dossier y del plan están sistemáticamente desfasadas** | FASE-A post-cierre, al inyectar N-A1 en el prompt de B. Cuatro casos verificados contra el código: **(1)** V6 cita `v4_diagnostic_generator.py:3189-3194`, que hoy es la **llamada a `detect_pains`**; el `except Exception: return brechas` + caché real está en **`:3197-3202`**. Esa cita falsa se repetía **12 veces en 6 archivos** del plan, incluidas **3 en el prompt de FASE-H** — la fase que iba a editarla. **(2)** V11 cita `:1952`; el comentario residuo D6 está en **`:1953`**. **(3)** `dependencias-fases.md` §3 atribuía a H2 «el `except Exception` de `_identify_brechas`» en `v4_proposal_generator.py`, archivo que **no define** ese método (`grep -rn "def _identify_brechas"` = 1 resultado, en `v4_diagnostic_generator.py:3116`). **(4)** El comentario de `regional_benchmarks.yaml` cita `v4_diagnostic_generator.py L2240-2311`; el dict real está en `:3263-3344` | Los 4 quedaron corregidos el 2026-09-03, pero la **clase** de defecto sigue viva: el plan tiene decenas de citas de línea sin verificar y cada fase que edita código las desplaza. La lección `revalidar-citas-de-código-no-revalida-premisas` cubre el sentido contrario (citas correctas ≠ premisas correctas); esto añade que **las citas mismas ya eran falsas** | **Regla para cada fase restante**: antes de editar una región citada, `grep`/`Read` para confirmar que la línea contiene lo que el plan dice. **FASE-VERIFY** audita las citas de los ACs que certifique. Evaluar que los prompts de fase citen **símbolos** (`def _pain_to_brecha`) en vez de números de línea: los símbolos no se desplazan |
| S16 | *(agregar los que abran VERIFY y las fases)* | | | |
...
| NRs certificados | 0 / 12 formalmente — **NR5 verificado en A** (848 passed / 2 skipped / 11 warnings; diff vs pre-cambio = 1 línea, la duración) |
| Hallazgos del dossier superados | **4** (V2, V3, V14, P10) de (6+4+8/3+16+8) |
| Lecciones capitalizadas | **6** de FASE-A (§8: L-A1 … L-A6) — VERIFY consolida el total |
