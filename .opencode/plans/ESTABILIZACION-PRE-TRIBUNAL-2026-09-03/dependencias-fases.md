...
| I | ✅ Completada | 2026-09-04 | **VERIFY** — única corrida E2E del plan sobre Salento Real: **14/16** checks contra baseline con los 2 deltas explicados, **0 archivos de código tocados**. Ocho seguimientos **S-I1…S-I8** a VERIFY | VERIFY |
| VERIFY | ✅ Completada | 2026-09-04 | **HOTFIX-PRE-RELEASE** — certificación con sonda propia sobre los artefactos de la corrida: **8 AC ✅ / 4 ⚠️ / 0 ❌** y **10 NR ✅ / 1 ⚠️ / 1 ❌**. Los 4 ⚠️ son de **serialización**, no de criterio ⟹ materializan la sesión extra. Write-back a QMind ejecutado (una fuente por plan) | HOTFIX |
| HOTFIX-PRE-RELEASE | ✅ Completada | 2026-09-04 | **RELEASE** — las 9 tareas (serialización, prosa de artefacto y plantilla): **11 AC ✅ / 1 ⚠️ / 0 ❌**. Cerrados **S-V1** (rojo), **S-V3** (AC6: `coverage_ratio` + denominador en la matriz `2.1`), **S-I2** (AC7: `severity`/`blocks_publication` en el `gate_report`), **S-V4/S-V5** (AC8-b reformulada como criterio de test), **S-I3** (mitad contract de la clave canónica) y la **mitad textual de S-C3**. Executor **v2.19.0** + `scripts/validate_plan_citations.py` como check 8 → validaciones **8/8**. **No** tocó umbrales ni listas de severidad (contrafactual **0 flips / 3 artefactos**). Código en `736eea8`. **Difiere con dueño**: S-HF1 (AC10), mitad estructural de S-C3 (P12) y S-I1 (NR2) → **tribunal** | RELEASE |
| RELEASE | ✅ Completada | 2026-09-04 | **Plan completo (11/11 + HOTFIX).** Versión **4.75.0** publicada con codename «Estabilización pre-tribunal». Cerró **S11/S-I6**: `AGENTS.md` 3.689/284 → **3.934 funciones / 298 archivos** (medido; la tabla por módulo quedó rehecha para que sus filas sumen el total) y `validate_agents_md.py` pasó de **5/6 a 6 PASS / 0 FAIL**. Publicó el **1 AC ⚠️ y el 1 NR ❌ como tales** (no como verde), copió `09` **§C.2** y no §C.1, y **0 rojos heredados** fueron a peor: contratos **180/0** y suite dirigida **950/2** re-ejecutados *después* de editar los documentos. **Extras hallados al medir**: la regla `agents_version_comment` de `scripts/sync_config.yaml` no aceptaba el prefijo `v` que exige `validate_document_integration.py` ⟹ la cabecera de `AGENTS.md` llevaba releases quedándose stale mientras el script decía «in sync» (corregido en el patrón, no a mano), y `README.md` conservaba la fecha legible de la release anterior (audit E8b). ⚠️ **No cerró S-H15/S-H16/S-H17/S-I8**: siguen abiertos porque **la suite ancha no la re-ejecutó nadie desde FASE-H** y el alcance de RELEASE declara «Comandos largos: 0» ⟹ nadie autorizó esa corrida. **Tampoco S-HF1 / mitad estructural de S-C3 (P12) / S-I1**: criterio de negocio, dueño el **tribunal** (DA-V5) |
...
| **VERIFY** | I | Certifica contra evidencia real de la corrida · **✅ ejecutada 2026-09-04** — 8 AC ✅ / 4 ⚠️ (AC6, AC7, AC8-b, AC10) / 0 ❌; 10 NR ✅ / 1 ⚠️ (NR12) / 1 ❌ (NR2); mediciones: 944/2, contratos 170 passed + **1 failed (S-V1)**, validaciones 7/7 | **HOTFIX** |
| **HOTFIX-PRE-RELEASE** *(sesión extra, no es la fase 11)* | VERIFY | **✅ Ejecutada 2026-09-04** (`736eea8`). Nueve tareas H1-H9, todas de **serialización, prosa de artefacto o plantilla**: cerró **S-V1** (rojo migrado al contrato, no xfail), **AC6/S-V3**, **AC7/S-I2**, **AC8-b/S-V4+S-V5**, **S-C3 mitad textual**, **S-I3 mitad contract** y midió **AC10/S-I7** ⟹ quedó ⚠️ **con delta y dueño**. Materializó en el executor (**v2.19.0**) las 4 reglas de VERIFY y la de citas como verificador (`scripts/validate_plan_citations.py`, check 8). **Difiere con causa (DA-V5)** la estructura de P12 (`score=1.0` hardcode + unión del denominador) y **NR2/S-I1** → dueño **tribunal** | RELEASE |
| **RELEASE** | VERIFY + HOTFIX | El CHANGELOG y GUIA_TECNICA se alimentan de `09` **§C.2 + §D** y `10`, ya completos. **Ya no hereda ningún rojo del plan**: el de `tests/delivery/test_delivery_contract.py::TestP05G9Gate` (**S-V1**, lo causó FASE-F) lo cerró el HOTFIX migrando el test al contrato de F. Lo suyo: **S11/S-I6** (sync de conteos con `sync_versions.py`), **S-H15/S-H16/S-H17/S-I8**, regenerar `DOMAIN_PRIMER.md`, mover `VERSION.yaml` a 4.75.0 y **refrescar la fuente QMind del plan con L-HF1**. Lo que **no** debe tocar: S-HF1 / P12 / S-I1 — criterio de negocio en manos del tribunal | — |

**Camino crítico**: A → B → C → F → G → H → I → VERIFY → **HOTFIX-PRE-RELEASE** → RELEASE (10 sesiones;
el hotfix es sesión extra, no fase del plan — **ya corrió**: sin él, RELEASE habría certificado 8/12 ACs
con 4 ⚠️ por serialización).
