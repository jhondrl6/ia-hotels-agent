...
> **Versión objetivo**: 4.74.0 | **Estado**: Preparación completada 2026-08-30 — pendiente ejecución de FASE-A
...
| FASE-B | 2 | Superficies secundarias: saneamiento `last_url` (AC6), hook-pdf (AC7), defensa capa datos (`assert_own_site` en scraper/auditor). **Nota auditoría F2**: `ensure_url()` se llama para TODOS los comandos en `main()` (main.py:1406, verificado); el tuple de L217 solo excluye la REINYECCIÓN, no la validación → no existen "modos sin ensure_url" y NO se agregan guards por modo | Baja-Media | **DELEGADO** — subagente 1 (main.py: `last_url` + contratos ensure_url por comando) ‖ subagente 2 (hook_pdf_generator + capa datos), parent integra | No |
...
| AC4 | `--force` (flag global reutilizado, main.py:172): bypass con evento persistido en el archivo dedicado `.agent/memory/url_guard_force_events.json` | A | FASE-C P9 + VERIFY |
...
- Línea base sucia: re-verificar los 12 tests rojos preexistentes (memoria 2026-08-29) al inicio de FASE-A y NO atribuirlos al plan; residuo `test_config_pricing` fuera de alcance (§7.1 del contexto). Verificado 2026-08-30: 13 fallos exactos en HEAD (3 calculator_v2 + 8 pricing_resolution_wrapper + 1 site_verification_propagation + 1 config_pricing).
- **`--force` YA EXISTE** (auditoría F1): `main.py:172` lo define global ("Sobrescribir PDF si ya existe", usado por hook-pdf L1565/1575). FASE-A lo REUTILIZA (prohibido `add_argument("--force")` de nuevo → argparse ArgumentError); se extiende el help a la semántica dual "bypass del guard / sobrescribir". `hook-pdf --force` omite ambos (documentado y testeado).
