...
| Probes Don Julio | 11/11 PASS | C |
| Coherence E2E Salento Real | **0.88** (= baseline H2 0.88), READY_FOR_PUBLICATION, 13/13 gates perfil idéntico | D |
| Run E2E FASE-D | EXIT_CODE=0, pared ~3 min (17:25:02Z→17:28:14Z); "Using defaults"; 7/7 checks `verificar_no_regresion.py` | D |
...
| `evidence/FASE-VUP-D/` | Evidencia E2E: docs comerciales + JSON de v4_audit + ZIP delivery + `run_stdout.log` + `verificar_no_regresion.py` + `verificacion_resultados.json` + `comparacion.md` | D |
| `output/FASE-D_salentoreal_post_guard/` | Corrida única E2E post-guard (artefactos completos v4complete) | D |
...
- **FASE-C (2026-08-31), modo DELEGADO (subagente probes + parent verifica)**: 11/11 probes PASS. P1-P4: rechazo OTA/red social exit 2, mensaje español, sin red ni artefactos. P5: estado envenenado sembrado → rechazo con mención "estado persistente" (AC6 verificado: `ensure_url` aborta antes de `save_state`, la URL sembrada permanece pero NO fue re-persistida por el proceso). P6: probe original usó `--report-json` (inexistente); corregido a `--output-dir` con fixture en directorio → exit 2 rechazo AC7. P7-P8: `UrlNoPropiaError` en scraper/auditor, 0 peticiones HTTP. P9: `--force` bypasea guard, evento persistido en `url_guard_force_events.json` (dict {timestamp, url, comando}). P10: 28/28 canonicalización. P11: 73/73 guard tests. Regresión dirigida: 101 passed 0 failed. Evidencia: `evidence/FASE-VUP-C/` + `resumen_probes.json`.
- **FASE-D (2026-08-31), modo MIXTO (comando delegado + parent verifica)**: preparación parent — git limpio, `clientes/` sin YAML de Salento Real (solo zi-one-luxury → defaults garantizados, F5), compile-check de las 5 superficies del guard, grep sin símbolos indefinidos, check offline de que el guard pasa `hotelsalentoreal.com`. Comando delegado a subagente: `v4complete --url https://www.hotelsalentoreal.com/ --output output/FASE-D_salentoreal_post_guard` → EXIT_CODE=0, pared ~3 min, "Using defaults (no fresh onboarding data found)" (log:166), 0 rechazos del guard durante el run. Protocolo de Evidencia Proactiva ANTES del análisis → `evidence/FASE-VUP-D/` (docs + JSON de v4_audit + ZIP + `run_stdout.log`). Verificación con `verificar_no_regresion.py` (parseo JSON UTF-8): **7/7 checks PASSED** — target_id `hotel_hotelsalentoreal.com`, coherence 0.88 (= baseline), READY_FOR_PUBLICATION, 13/13 gates perfil idéntico sin regresión, plan de assets byte-equivalente, pains→assets idéntico, escenarios financieros byte-equal (6,571,622.4 / 4,042,752.0 / 1,264,435.2 COP/mes). Anomalías clasificadas infraestructura preexistente (L14): gemini 403 (LLM narrativo opcional) y PageSpeed "API key not valid" (también `skipped` en el baseline H2). Duración del run: ~3 min de pared. AC3/AC8 verificados empíricamente. Evidencia: `evidence/FASE-VUP-D/` + `comparacion.md` + `verificacion_resultados.json`.
