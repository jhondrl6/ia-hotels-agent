...
| Imports del SDK/adapter fuera de la puerta (AC6) | **0** sobre 678 `.py` rastreados / **692** del árbol de trabajo (690 en el primer escaneo: +2 por los instrumentos propios) (medido al abrir FASE-B con el AST de la propia puerta) | **0** re-medido al cerrar (misma población) | El SDK sí existe en el entorno, pero **aislado en `tmp_test/venv-jev-sdk`** y no instalado en el venv del producto; esa exclusión está publicada con su conteo en `import_scanner.txt`, no callada |
...
- [ ] `.opencode/LECCIONES-INDEX.md` y `.opencode/lecciones_index.json` (regenerados en cada commit que escribe `.md` de plan con IDs) — **FASE-B regeneró el par el 2026-09-21** al escribir sus seis documentos de cierre; el commit que lo lleve dentro pide instrucción literal
