...
| Índice de lecciones — **D añadió corpus al índice y eso lo vence** | Los packs generados son `.md` **dentro del corpus que escanea `build_lesson_index.py`** (viven bajo `.opencode/plans/<PLAN>/briefing/`), así que el acto de generarlos copia IDs y **vuelve a vencer el par**: es la medición A6 del maestro cayendo sobre un artefacto derivado, no un defecto del generador. Por eso el paso 5 del contrato obliga a regenerar el índice **sobre el mismo árbol final verificado**, y por eso el commit del plan tiene que llevar el par dentro. ⟦Medido al cerrar D: `--check` da FAIL sobre un árbol limpio extraído con `git archive HEAD` y pasa a `exit 0` al regenerar **en ese árbol** (S15, `fuente_fecha = mtime`) — commitear el par desde otro árbol **no** lo cura. ⟧ **Ese límite se curó el 2026-09-26 y ya no describe el árbol: `scripts/build_lesson_index.py` dejó de consultar el `mtime` para fechar** — tres fuentes admitidas (nombre del plan, último commit que tocó su documento, estado explícito `SIN-FUENTE`)—, así que un `[FAIL]` de índice visto en un clon o en una extracción limpia **deja de ser S15** y hay que leer la línea `[fechas] nombre=… commit=… sin_fuente=…` que el `--check` imprime en verde **y** en rojo. Dos advertencias para quien lea esta tabla después de la cura: un clon de este repo se hace con `-c core.autocrlf=input` (el `system` es `true` y `git clone` no lee la config local), y en un test que clone el repo el checkout completo truena por rutas largas (`--no-checkout` + `git checkout HEAD -- .opencode/plans .opencode/context`). Su fila y su prueba son la fuente única: **S15** en `10-analisis-post-implementacion.md` §Seguimientos y `tests/test_build_lesson_index_s15_fecha_versionada.py`; la cuenta de IDs tampoco se re-transcribe aquí, la imprime el propio `--check`. **Lo que esta fila NO venció:** regenerar el par **sobre el mismo árbol final verificado** sigue siendo obligatorio, porque escribir corpus sigue venciendo packs e índice, y el orden del cierre sigue siendo escrituras → packs → par → los dos `--check` → `--quick` (**L-VCF-17**). Antecedente: el par `.md`+`.json` **viajó dentro de `647f436`** (`--numstat`: `LECCIONES-INDEX.md` 24+/17−, `lecciones_index.json` 100+/14−), así que `[6/7]` del hook ya no corta este cierre. **330 IDs** definidos al cerrar B y **332** re-medidos el 2026-09-22 tras el barrido de citas (sube porque el propio barrido define L-VCF-11 y L-VCF-12 — A6 otra vez: la cifra caduca al escribirla); re-medir al abrir D, y volver a medir al cerrarla: la escritura documental de la propia fase mueve la cifra (A6) | `python scripts/build_lesson_index.py --check` |
