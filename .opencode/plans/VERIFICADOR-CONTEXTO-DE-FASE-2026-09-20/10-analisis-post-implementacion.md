...
| FASE-B | **VERIFICADO OFFLINE 2026-09-21** (AC6–AC9; AC16/AC17/AC18 en su parte de B) | Instrumento canónico **volvió a no correr**: `find . -name "*.jsonl"` = **0** en el workspace, medido hoy (precondición de D-V2.1). La métrica se **retira**, no se estima: no se publica un `tool_use` aproximado, porque contar bloques desde dentro de la sesión no es medible sin el transcript que falta. Unidad contable en disco, declarada no comparable: **11** archivos de código/tests (1 puerta + 2 proveedores falsos + 6 de tests + `conftest.py` + `__init__.py`) y **25** de evidencia (10 de ellos en `mutation/`, 2 en `instrumentos/`), ambos contados con su comando en `baseline-pre-post.md`, no estimados | Ninguno: 3 tareas de código y **0** comandos de larga duración (R3) | Evidencia en `evidence/VERIFICADOR-CONTEXTO-DE-FASE-2026-09-20/FASE-B/`: `informe.json`, `import_scanner.txt`, `contract.txt` (verde `exit 0` + rojo `exit 1`), `costura.json`, `extensibilidad.txt`, `cero-red.txt`, `run_tests.txt` (53 casos), `baseline-pre-post.md` con la resta y `mutation/` (verde + 9 mutantes). **Commiteada el 2026-09-22 en `647f436`** (46 archivos, +4.412/−97) con instrucción literal del operador y con el par del índice **dentro**; los **7** checks del pre-commit pasaron. Dos cosas quedan declaradas y no cerradas: la ruta ajena `EVALUACION-JEV/dependencias-fases.md` **excluida** a propósito del commit, y el **push sin autorización** (paridad `0/2` al commitear, hoy **`0/3`** tras el barrido `612efd0` del mismo día — cada commit documental suma uno: re-medir —, con el commit ajeno `eecf246` por delante) |
| FASE-C | PENDIENTE | — | — | — |
| FASE-D | PENDIENTE | — | — | — |
...
| S11 | **El denominador de AC6 admite un archivo de entorno no declarado**: `.venv-wsl/bin/activate_this.py` entra en los 692 `.py` que escanea `iterar_py()` porque `.venv-wsl` no está en `ARCHIVOS_EXCLUIDOS_DE_LA_POBLACION`. Y las exclusiones se solapan (`site-packages` anidada en `venv`/`tmp_test`), así que su suma no reproduce el universo. El numerador no se mueve: **0** imports prohibidos re-medido el 2026-09-22 | Quien toque `decision_client.py` por otra razón (D7/S10) o FASE-RELEASE al consolidar | Que exista una re-ejecucion de la puerta sobre el árbol vigente: añadir la exclusión **y** su test de población (la exclusión se publica con conteo, no se calla), o publicar el residuo como límite del instrumento. No se hace en un barrido documental: exige re-correr los 53 casos y los 9 mutantes (L-VCF-11) |
| S12 | **`validate_governance_numbers.py --report` escribe por default dentro de la evidencia commiteada de FASE-A** (`REPORT_DEFAULT`, línea ~70): cada corrida re-escribe `generated_at`, `medido_el` y el conteo inyectado de 4.330 a la cifra vigente, o sea **pisa el registro fechado de otra fase**. Medido y revertido el 2026-09-22 (`git checkout --` sobre ese archivo; re-muestreo con destino explícito deja FASE-A limpia y el mismo `HALLAZGOS` con `exit 1`) | FASE-RELEASE de este plan (es edición de `scripts/`, no de `.agents/`, así que no choca con AC17) o quien toque el script antes | Al consolidar: `--report` sin destino debe imprimir **sin escribir** (o exigir `--out`), y el comando que se publica en los documentos lleva su ruta de evidencia. Hasta entonces: nunca correr el default sobre el árbol vigente (L-VCF-12) |
