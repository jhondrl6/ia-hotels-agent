...
| 3a | **FASE-P3-A** | Detección y fidelidad: AC-F1 (dos capas, lectura **desde el ZIP** + stub estructural), AC-F2 (fuente del tier pre-packaging), AC-F4 (primer piso en `B_PLUS`) — **primera fase de ejecución** (orden DA-P1.3) | MEDIA | DIRECTO | ⬜ Pendiente |
| 3b | **FASE-P3-B** | Cableado y test: **AC-F5 disparado** (banderas reales de analítica, toca `main.py`), AC-F3 (whitelist barreda justificada), AC-F6 (versión del acta desde `VERSION.yaml`) | MEDIA (era BAJA; Q5=a la sube) | DIRECTO | ⬜ Pendiente |
