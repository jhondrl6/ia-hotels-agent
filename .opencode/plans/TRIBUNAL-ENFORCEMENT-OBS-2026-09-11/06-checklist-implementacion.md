...
**6/8 fases cerradas** (sin contar el predecesor ni VERIFY, que no está activa). P4 permanece cerrada y **P5 cerrada el 2026-09-15** con remediación post-auditoría el mismo día. Orden restante: **FASE-P6 Generación y validación multi-hotel → RELEASE documental**. AC-V1 se ejecuta dentro de RELEASE, sin sesión VERIFY adicional.
...
| 5 | **FASE-P5 — Seguridad y privacidad** | ✅ **Cerrada 2026-09-15** (técnico) | 2026-09-15 | 2026-09-15 | presupuesto **FUERA DE SERVICIO** (R2.1/D-V2.1, sin estimación); medida real auto-reportada con unidad declarada: **1 commit de ejecución (`b25b63a`) + 1 de remediación, corte = HEAD al cerrar**; el instrumento no alcanza el transcript (D-V2.1 reproducido, igual que P2). Celda antes vacía → se cierra en la remediación (L-P5.3) | AC-S1 ✅ (key a header `x-goog-api-key`, `_sanitize_text`/`_sanitize_error`, 11 tests, par NR7); AC-S2 ✅ **remendado** (el escaneo staged estaba muerto por NameError tragado — L-T2C.2 en el propio checker; alcance tracked+staged con sniff NUL, estados NR8 reales, `NO_CUBIERTO` bloqueante); AC-S3/S4 ✅ inventario + puerta con dueño. **Nuevo check `[5/10]` de material de cliente separado del de claves** (`config/client_material_policy.yaml`, 1 grandfathered con dueño y pendencia AC-S4). NR1: 4.145→**4.155 (+10)**, `--quick` **10/10**. Detrás de la publicación sigue la puerta AC-S4 (decisión del operador). Evidencia: `evidence/FASE-P5/REMEDIACION-auditoria-2026-09-15.md` |
| 6 | **FASE-P6 — Generación y validación multi-hotel** | Pendiente | — | — | Pendiente; presupuesto FUERA DE SERVICIO (R2.1/D-V2.1) | Depende del cierre técnico P5; 4 tareas, 0 comandos largos, offline; AC-G1…AC-G5. No exige ni autoriza otra corrida real |
| — | FASE-VERIFY | ❌ **No activa** (decidido en P1) | — | — | n/a | AC-V1 de RELEASE certifica AC-E*/AC-F* existentes y AC-S1…AC-S4/AC-G1…AC-G5 nuevos contra artefactos y pares NR7; sin sesión adicional |
| 7 | FASE-RELEASE-4.77.0 | ⬜ Pendiente | — | — | — | Después de P6: cierre documental + AC-V1 + puerta de seguridad/privacidad; tag/publicación/archivado solo bajo el alcance y las autorizaciones de esa sesión |
