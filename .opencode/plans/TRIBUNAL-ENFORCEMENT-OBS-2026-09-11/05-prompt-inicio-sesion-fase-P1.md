...
| Deuda de gates medida en ese R2.5 + Paso 0 horizontal | ⚠️ **Heredada a P1** — **9 ítems** en §Deuda de proceso de `06-checklist-implementacion.md`: 6 del R2.5 (R2.6/R2.7 sin verificador mecánico + baseline de R2.6 fuera del repo, `validate_plan_closure.py` 1/8, `Version actual` del REGISTRY escrita a mano, reescrito ciego de `validate_opencode_refs.py --fix`, regex de `version_consistency_checker.py`, y L-R.1 — **esta última ya aplicada a este plan**) + 3 del Paso 0 horizontal de 2026-09-12 (Paso 0 sin verificador, normalización del flaky de orden en R2.7, Tier A inalcanzable en `v4complete`) |
| FASE-P1 | ← ESTA FASE |
...
**Consultas ejecutadas del Paso 0**: las 8 literales están en §1 de `00-lecciones-capitalizadas.md` (Q1–Q5 al notebook `iah-cli-lecciones` vía `qmind retrieve --nb 01a04d98-b7bd-778c-8441-26fdc7e35f45`, narradas también en §9 de `01-plan-maestro.md`; Q6–Q8 al índice generado `.opencode/LECCIONES-INDEX.md`). Límite: el Paso 0 no tiene verificador mecánico, así que esa lista es la declaración exigida por L-R.4, no la prueba de que se cumplió.
...
| **Q4** | ¿Hotel y datos para P4? (precondición T3a) | Quién provee `rooms`/`occupancy_rate`/`direct_channel_percentage`/`ADR` con fuente; URL propia; consentimiento |
| **Q5** | ¿Cómo se resuelve que Tier A sea inalcanzable en `v4complete` (§2.1)? | (a) P3 propaga las banderas reales al `HotelFinancialData` del bloque FASE-K (AC-F5, toca `main.py`) · (b) P4 se corre en `B_PLUS` declarando el techo · (c) P4 se difiere a un plan de analítica y este plan cierra sin corrida |
...
- [ ] Si Q5=(c): el README pasa a 4 fases y `dependencias-fases.md` elimina P4 (con la decisión registrada, no en silencio)
...
6. Plan de P2/P3 actualizado con lo decidido (qué fase hace qué, incluido AC-F5 si Q5=a)
...
6. **`10-analisis-post-implementacion.md`** de este plan: crear con Resumen de Ejecución + lecciones (mínimo 3) + la sección "Lecciones capitalizadas de planes anteriores" con la tabla del §9 del plan maestro *(el executor lo exige desde la concepción del plan; a 2026-09-12 no existe en este directorio — P1 lo abre y deja constancia del retraso)*
7. **`09-documentacion-post-proyecto.md`**: crear con la estructura base (tampoco existe, y FASE-RELEASE lo acumula desde ahí)
...
- **Q1b, Q5 y Q6 se cierran en esta sesión**: no se delegan a P2, P3 ni al brief de P4 (L-VUP-6 — delegar solo lo ya decidido)
