...
2. **Calificación y cierre mínimo.** Reutilizar la sanitización existente y corregir únicamente rutas descubiertas que permitan persistencia o impresión sin redacción. Definir el contrato que consumirá H, sin construir aquí su runner. ~~Solicitar acreditación operativa de revocación mediante metadatos no secretos~~ **RESUELTO (2026-09-19): el operador acreditó la rotación de la key Gemini-local (AIzaSyDq…) el 2026-09-18; registro en `evidence/FASE-P5/AC-S3-S4-inventario-superficie.md`. F la referencia como afirmación del operador, no la reconstruye ni la infiere de tests; la key de `archives/gbp_profiles.json` no es credencial del operador (higiene cerrada 2026-09-19)**; registrar estado real sin rotar credenciales ni contactar servicios.
