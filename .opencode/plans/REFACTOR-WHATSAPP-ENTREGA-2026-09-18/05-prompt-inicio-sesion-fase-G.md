...
Actualizar estado de este prompt, checklist, dependencias, índice del plan, 00/09/10 con las observaciones sustentadas que hayan existido, **sin cuota**: declarar «sin lecciones nuevas» es un resultado válido y se registra con esa palabra, no es un hueco que rellenar ⟦**Antecedente barrido el 2026-09-26** — esta línea mandaba «00/09/10 con **al menos tres observaciones medidas**». Es la instancia que el §Lecciones nuevas de `10-analisis-post-implementacion.md` de este plan dejó nombrada y sin barrer el 2026-09-25, porque caía en el prompt de una fase cerrada y quedaba fuera del permiso de aquella sesión. La cuota la retiró el **bloque B** de `ORDEN-CAMBIO-CALIDAD-PROCESO-2026-09-22.md` con su regla de permitir declarar que no hubo lecciones nuevas sin fabricar una cuota. El número **no** se sustituyó por otro número. Esto no reabre FASE-G: el encabezado de este prompt declara la fase COMPLETADA y commiteada el 2026-09-20, así que la barrida es de veracidad del registro y no una ejecución pendiente⟧; subsección G en CHANGELOG bajo versión vigente y nota en `docs/GUIA_TECNICA.md`. Sustituir variables por datos medidos; registro propio sin `--release`:
