...
1. **PRE e integración offline.** Medir baseline antes de cambios. Descubrir parser, `_load_latest_onboarding_data`, `_observation_to_onboarding_format`, `_normalize_url` y writers/lectores de E/F. Recorrer el flujo productivo con servicios externos sustituidos y red prohibida; comprobar permitir/bloquear sin invocar la CLI real. Verificar prerrequisitos de identidad, revocación y vigencia.
...
- Cambiar solo el vínculo `hotel.url` del derivado a `https://www.donalfonsohotel.com/`, URL indicada por el usuario; original `https://hoteldonalfonso.com/` permanece intacta.
...
- No inventar `--onboarding-file`, usar `--output-dir` de hook-pdf ni buscar el último archivo entre hoteles. Confirmar identidad `hotel_don_alfonso` mediante reportes.
