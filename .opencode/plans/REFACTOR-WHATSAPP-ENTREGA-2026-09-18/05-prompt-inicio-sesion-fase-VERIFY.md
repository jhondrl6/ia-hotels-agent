...
2. **Dictamen por AC y certificación.** Recorrer AC1–AC19 contra artefactos reales y clasificar cada uno: **SUPERADO** (offline y/o E2E, indicando el régimen), **FALLA** o **NO EJERCITADO**. Emitir `certificacion.json` con matriz, evidencia citada por símbolo/ruta, límites y diff estructural del cambio (superficies tocadas por fase frente a lo planificado). READY_FOR_PUBLICATION exige gates y acta favorables y ZIP válido: `exit_code == 0` no lo acredita. Si la meta de entrega no se demuestra, declararla parcial/FALLA para esa meta.
...
- [ ] Matriz AC1–AC19 dictaminada con régimen offline/E2E explícito y artefacto citado por AC.
