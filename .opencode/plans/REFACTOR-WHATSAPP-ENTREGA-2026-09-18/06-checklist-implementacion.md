...
|---|---|---|---|---|
| A | Mandato de sesión | Baseline, permisos, matriz ratificada (incluida FASE-0 y AC19a/AC19b), identidad/vigencia; AC14 | **CERRADA 2026-09-19 (commit `3e97d95`, en paridad con `origin/master`)** | `evidence/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/FASE-A/`: `decisiones.md`, `baseline_inventory.json`, `qmind-consulta-reintentada.md`, `tests_pertinentes_pre.txt` |
| G | A cerrada | AC7, AC16, AC15. **Guard de las ediciones de B–F** | **CERRADA 2026-09-20 — commit `66e17bd` y push a `origin/master` (paridad 0/0)** (código + cierre documental) | `evidence/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/FASE-G/`: `wiring_report.json`, `inventario-callers.md`, `resultados-y-observaciones.md`, `premediciones.md`, `qmind-eje-cierre.md`, `tests_baseline_pre.txt`, `tests_baseline_post.txt`, `mutation_report.json` (7/7 con rojo por el guard) |
...
| AC7 | G; VERIFY contrasta | `wiring_report.json` expone población descubierta de validate/detect_pains, kwargs, fuentes, excepciones tipadas y conexión al quick | Caller nuevo en archivo nuevo, señal omitida/kwargs opacos hacen rojo; cubrir aliases/self y excluir validate ajenos | **VERIFICADO OFFLINE (G, 2026-09-20)** — quick 11/11 con el check registrado; rojos medidos: caller nuevo en archivo nuevo, `**kwargs` opacos, alias de import, `self.<attr>`, homónimos excluidos, **y la divergencia vigente en TRES invocaciones de `v4_asset_orchestrator.py` antes de corregirla** |
...
| AC15 | Todas las implementaciones/H; VERIFY contrasta | PRE/POST con misma selección/entorno, exit codes y delta explicado; `mutation_report.json` por AC, guard y test | PRE antes de editar tests; rojo causado por guard y no syntax/import; no ocultar regresiones ni mezclar funciones con casos parametrizados | **VERIFICADO OFFLINE (G, 2026-09-20)** — selección literal en el txt; PRE 1.313/1/2 y POST-A idéntico (delta 0); POST-B 1.331 (+18 del suite nuevo); **7/7 mutaciones con rojo por el guard, 0 por syntax/import**; el rojo preexistente de otra fase se conserva igual en PRE y POST en vez de excluirse |
| AC16 | G; VERIFY contrasta | Inventario AST de with_validation sin argumento descartado; check legacy clasificado; ValidationSummary conserva su dato upstream | Caller con firma vieja hace rojo; no crear verdad paralela whatsapp_validation ni borrar variable todavía consumida | **VERIFICADO OFFLINE (G, 2026-09-20)** — firma leída por el verificador `with_validation(self, validation_summary)`; rojos M5 (firma) y M6 (caller); las 8 líneas upstream de `main.py` intactas; `domain_gates` clasificado sin tocarlo |
...
**Resumen tras FASE-G (2026-09-20):** G **CERRADA** con código y cierre documental. `scripts/validate_wiring.py` gobierna cuatro productores por AST (AC7) y el contrato muerto quedó retirado (AC16); el modo rápido pasó de 10 a **11 checks** y da 11/11; **18 funciones canónicas nuevas** (conteo del repo 4.246 → **4.264**); contador v4complete **0/1**. **Quedan abiertos con dueño:** los tres hallazgos de cableado de `v4_asset_orchestrator.py` (FASE-B/AC1, amparados por excepción tipada que se auto-invalida al corregirse), el fallo preexistente `test_medido_contra_el_predecesor_entra_en_alcance…` (causa medida: `9c4a001` archivó el plan y `clasificar_planes` solo mira hijos directos de `.opencode/plans/`), el commit y el push de G, **ejecutados el 2026-09-20 con autorización literal** (`66e17bd`, paridad 0/0), y su write-back a QMind, que sigue sin autorización. **Siguiente sesión: FASE-0.**

**Resumen tras FASE-A (2026-09-19):** A **CERRADA** con commit y push (`3e97d95`); once sesiones PENDIENTES; AC1–AC20 siguen PENDIENTES (A no certifica ninguno: certifican las fases dueñas y VERIFY); intento **0/1**; siguiente sesión **G**. De los prerrequisitos de entrada, A cerró con medición o decisión: quick re-medido 10/10, baseline preservado (0 escrituras), AC19 dimensionado por tercera vez, DOMAIN_PRIMER resuelto sin editar documentos centrales, identidad/URL con redirección observada, revocación acreditada por referencia y QMind recuperado. Commit y push del cierre ejecutados (`3e97d95`). **Quedan abiertos con dueño:** la reconfirmación de vigencia/consentimiento (operador, antes de H/E2E) y, más abajo, F-B (decisión escrita de privacidad). El quick histórico 9/10 no se rebautiza verde: se midió de nuevo.
