...
**Estado: FASE-G CERRADA el 2026-09-20 y empujada el mismo día (primera sesión con código de producto; commit `66e17bd` en `origin/master`, paridad 0/0); 0–RELEASE pendientes.** **Contador v4complete: 0/1. Siguiente sesión: FASE-0.**
...
Copiar en una sesión nueva. **Nota de higiene:** este bloque seguía diciendo «FASE-A» después de que A cerrara; lo venció el propio cierre. Se reescribe en cada fase y su frescura la mira `validate_plan_closure.py`.

```text
Ejecuta únicamente FASE-0 del plan C:/Users/Jhond/Github/iah-cli/.opencode/plans/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/. Lee 05-prompt-inicio-sesion-fase-0.md, 01-plan-maestro.md §2 (FASE-0 nueva y el límite exacto de «enforcement invariante») y §4 (AC20 y AC12 en su rama publish), 04-contrato-ejecucion.md, 00-lecciones-capitalizadas.md (§1bis, §2 y la fila L-E2E.2), dependencias-fases.md, 06-checklist-implementacion.md y el workflow canónico. Heredas de G (cerrada 2026-09-20): el quick son ahora 11 checks, con `Wiring` de guard (reporta, no reescribe); la política AST gobierna 4 productores; TRES invocaciones de `modules/asset_generation/v4_asset_orchestrator.py` (medidas en 286, 309 y 447) omiten `whatsapp_html_detected` con dueño FASE-B/AC1 y excepción tipada que se auto-invalida si B corrige solo una; `AssessmentBuilder.with_validation` ya no recibe `whatsapp_validation` y el dato upstream de `main.py` sigue vivo. El trabajo de G ya está **commiteado y empujado** (`66e17bd`, paridad 0/0 con `origin/master`): no lo reviertas ni lo vuelvas a stagear. Hay un rojo preexistente con dueño y causa medida: `test_medido_contra_el_predecesor_entra_en_alcance…`, porque `9c4a001` archivó el plan y `clasificar_planes` solo mira hijos directos de `.opencode/plans/`. Re-mide antes de la primera tarea: git HEAD/status, `run_all_validations.py --quick` (11, ya no 10) y el baseline `output/TAREA7-2026-09-19/`. AC20 cubre DOS ramas de publicación sin `package_evidence`: `packager.publish()` y el `except` never-block de `main.py`. No toques `TribunalJudge._compute_verdict`, flags de bloqueo, umbrales ni `write/publish/suppress`; prohibido el verde barato por severidades, `BLOCKING_VERDICTS` o `GATE_BLOCKING_ENABLED`. No inicies B ni ejecutes v4complete. Conserva el contador 0/1 y deja checkpoint si falta autorización.
```
