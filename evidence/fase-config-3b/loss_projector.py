...
class LossProjector:
    ...
    def __init__(self, hotel_name: str, projection_months: int = 12):
        ...
        self._degradation_rate = LossProjector._load_degradation_rate()

    @staticmethod
    def _load_degradation_rate() -> float:
        """Load degradation_rate from config/scenarios.yaml. Falls back to 0.02."""
        try:
            config = load_yaml_config('scenarios')
            return float(config.get('degradation_rate', 0.02))
        except (YAMLLoadError, KeyError, TypeError, ValueError) as e:
            logger.warning(f"scenarios.yaml unavailable, using degradation_rate=0.02: {e}")
            return 0.02
