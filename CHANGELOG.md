...
**Fix posterior al cierre de la fase (2026-09-20, instrucción expresa del operador)**: `VERSION.yaml` recibe `date: "2026-09-19"`, igual a `release_date`. Sin esa clave, `scripts/sync_versions.py` la inyecta con `datetime.now()` y el check de Version Sync caduca a las 00:00 de cada día: al pasar la medianoche marcó FAIL en cuatro encabezados (`AGENTS.md`, `.cursorrules`, `docs/GUIA_TECNICA.md`, `docs/contributing/REGISTRY.md`) que estaban correctos para la release, con quick 9/10 y ninguna versión tocada. Se ancló el valor en la fuente única en lugar de reescribir configuración central. Verificado: `sync_versions.py --check` 7/7 in sync, `version_consistency_checker.py` sincronizado, 7 passed en los dos tests que leen `VERSION.yaml` y quick 10/10. El script sigue dependiendo del reloj cuando `date` falte — eso queda declarado como límite, no arreglado aquí.
...
#### FASE-0 del plan REFACTOR-WHATSAPP-ENTREGA-2026-09-18 — evidencia del veredicto serializada (2026-09-20)

**Objetivo**: que la evidencia del veredicto sea completa y legible en **las dos ramas** de la decisión, para que el ZIP pueda publicarse cuando el producto lo merece y para que AC10/AC12/AC18 tengan de dónde leer. La causa raíz estaba medida: la corrida `output/TAREA7-2026-09-19/` terminó `BLOQUEADO` con el ZIP suprimido **sin que ningún gate hubiera fallado**, por un único hallazgo CRITICAL (`VACUOUS_RECALL`) nacido de un hueco de serialización del propio gate.

**Camino identificado por símbolo, no por línea**: el `1.0` de la corrida sale de `PublicationGatesOrchestrator._extract_critical_recall` en su rama `return 1.0  # All critical issues were detected` — recall **fundado** que viajaba con `details: {}`. Reconstruido con el `AssessmentBuilder` real sobre el `audit_report` archivado, el gate reproduce `value: 1.0, details: {}` (`coincide_con_el_archivo: true`). Y `main._build_gate_report_payload` ya serializaba `"details": r.details` sin filtrar: el writer del reporte **no** se tocó.

**Cambios** — tres, todos aditivos, y ninguno roza la regla de decisión:
- **Anotación del productor** (`_critical_recall_details`, nuevo): el camino fundado declara `critical_issues_count` y `recall_basis: all_critical_issues_detected`; si el audit evidencia críticos que la lista no cubre, `evident_critical_issues_missed` más `evident_critical_missed`. SR-H2 conserva `audit_present_no_critical_issues` y los casos no fundamentados siguen con `details: {}` **a propósito**: ahí la ausencia es la señal que L-SR5 manda preservar, y el detector `_check_vacuous_recall` queda intacto.
- **Causas en el acta** (`ReviewerReport.to_dict`): proyecta `findings` con lista blanca (`finding_type`, `severity`, `clause`, `description`), texto acotado a 240 y tope de 20 por revisor con `findings_omitted`. Medido sobre el acta real: JSON **+1.151 bytes (+84 %)**, **MD +0 bytes**, contra 9.155 bytes de los cuatro `revision_*.json` — resumen, no copia.
- **Evidencia del paquete entregado** (`main._record_published_package_evidence`, never-block por dentro): `package_evidence` con `suppressed: False`, `sha256`, `member_count` y `path` de la **ruta publicada** (`publish()` es `rename`, mismos bytes), en las **dos** ramas de publicación — la normal y el `except` never-block del enriquecimiento. Los contratos `write`/`publish`/`suppress` intactos.

**Contrafactual y mutaciones**: `TribunalJudge._compute_verdict` ejecutado en memoria sobre copia temporal del acta archivada (el `output/` del baseline re-verificado intacto): `BLOQUEADO` → `APROBADO-CONDICIONAL-PENDING-ONBOARDING`, veredicto que **no** está en `BLOCKING_VERDICTS`. Y la advertencia del plan se confirmó como hecho: hacer cero del `critical_count` **sin** recalar la `verdict_recommendation` sigue `BLOQUEADO`, porque `ReviewerReport.verified_block` decide por la recomendación. **6/6 mutaciones con rojo causado por el guard**, incluidas las dos prohibidas por el plan (M5 relajar el detector, M6 sustituir la serie SR-H2) — si alguien las intenta, algo rompe; restauración verificada por sha256.

**Archivos Nuevos**: `tests/test_fase_0_ac20_evidencia_veredicto.py` (21 funciones canónicas), `evidence/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/FASE-0/` — `resultados-y-observaciones.md`, `pre_camino_gate.py`/`.json`, `contrafactual_ac20.py`, `baseline_vs_contrafactual.json`, `run_mutations.py`, `mutation_report.json`, `build_thresholds.py`, `thresholds.json`, `tests_baseline_pre.txt`, `tests_baseline_post.txt`, `tests_postfull_regresion.txt`.

**Archivos Modificados**: `modules/quality_gates/publication_gates.py`, `modules/quality_gates/tribunal/outcome.py`, `main.py`, `tests/quality_gates/tribunal/test_diagnosis_reviewer.py` (fixture contradictorio corregido), `tests/quality_gates/tribunal/test_p2_veredicto_enriquecido.py` (aserción de forma re-atada a coherencia interna), y los seis documentos del plan.

**Tests**: selección literal de 12 archivos: PRE **189 passed / 1 skipped** y POST-B **189** con el código de la fase aplicado (**delta 0**); POST-A **210 passed / 1 skipped** (exit 0). La suite nueva contra `git archive HEAD` rompe **16** (13 por aserción, 3 por `ImportError` del helper nuevo, declarados). Regresión completa: **3 failed, 4.247 passed, 41 skipped, 4 xfailed** en 232 s — los tres rojos preexistentes con dueño, ninguno de esta fase. Funciones canónicas **4.264 → 4.285 (+21)**. Quick **11/11**. **Contador v4complete 0/1**: ninguna ejecución real. **Commit `7c6e75f` y push a `origin/master` ejecutados el mismo 2026-09-20 con instrucción literal del operador** (paridad 0/0 verificada con `git ls-remote`; los 7 checks del pre-commit pasaron sin saltar ninguno).

#### FASE-B del plan REFACTOR-WHATSAPP-ENTREGA-2026-09-18 — promesas realizables y pains coherentes (2026-09-20)

**Objetivo**: que la ausencia de un número de WhatsApp verificable deje de prometer un botón operativo, y que los tres productores que decían cada uno lo suyo sobre el mismo hecho lleguen a la misma respuesta. El punto de partida medido: `no_whatsapp_visible` prometía `whatsapp_button` en el mapper mientras `ConditionalGenerator.PAIN_TO_ASSET` ni siquiera tenía la clave (pain y asset decidían aparte), `get_assets_for_pain` forzaba `can_generate=True` para el conflicto, la guía de conflicto **elegía número por cantidad de reseñas**, y una huella de plugin en la raíz promocionaba el pain a `VERIFIED_IN_SITE` con severidad LOW.

**AC1 (cable, no fuente nueva)**: `V4AssetOrchestrator.generate_assets` deriva **una vez** `whatsapp_html_detected` de `audit_result.validation` y la propaga a `detect_pains` y a las dos llamadas de `CoherenceValidator.validate` (pre-gen y post-gen). Las tres excepciones tipadas que amparaban esas omisiones se retiraron de `scripts/validate_wiring.py`; sin hallazgo que amparar habrían caído en `EXCEPCION_VAGA`. Medido por el propio guard: 174 llamadas / 614 archivos, gobernadas 75 con **conformes 21, omisiones 0, violaciones 0**.

**AC2 (promesa condicionada)**: nuevo entregable `whatsapp_setup_guide` — catálogo, generador (`modules/asset_generation/whatsapp_setup_guide.py`), dispatch en `ConditionalGenerator`, identidad de servicio `guia_configuracion_whatsapp`, nombre comercial en `ASSET_NAMES` y clave en `PAIN_TO_ASSET`, que faltaba. Su contenido no lleva número ni `wa.me` ni placeholder, y no afirma que el hotel no tenga WhatsApp. El botón queda alcanzable **solo** por `whatsapp_conflict`, y sin `can_generate` forzado: la barra del mapping manda. La guía de conflicto dejó de seleccionar número: presenta candidatos y exige validación humana. `PAIN_TO_PRESENCE_ASSET["no_whatsapp_visible"]` pasa a la guía, así que una huella de plugin ya no verifica el canal.

**AC19a-consumo**: `PainLedger.STATUS_NOT_VERIFIED_IN_SITE = "NO_VERIFICADO_EN_SITIO"` se registra en `evidence_refs` (no en `status`, que consume el gate de cobertura), y la narrativa «Canal Directo Cerrado (Sin WhatsApp)» pasó a «Canal de WhatsApp sin verificar» con pedido de confirmación. Se corrigió además el comentario `FIX-D7` de `main.py`, que seguía presentando `whatsapp_button` como `promised_by=always` — condición que el catálogo eliminó en FASE-5.

**A1 (allowlist extendida con autorización del operador)**: en `proposal_asset_alignment.py` se separó la **tabla de resolución** (`RESOLUCION_SERVICIO_A_ASSET`, todos los servicios del registro) del **universo contado** por el gate (`PROPOSAL_SERVICE_TO_ASSET`, solo `counts_in_alignment`). El gate no cambió de denominador ni de regla. Se midieron y descartaron dos alternativas: contar la guía degradaba la cobertura de hoteles sin brecha (15 rojos), y ampliar el universo de la matriz con los condicionales hacía que `ProposalAssetMatrix.build(["Servicio Inexistente", …])` devolviera filas que nadie pidió (rojo en `test_unknown_service_is_skipped`).

**Mutaciones**: 8/8 (M1–M8) con rojo causado por el guard, ninguna verde persistente, restauración de los archivos mutados verificada por sha256 (`run_mutations.py` + `mutation_report.json`). Lección del entorno: restaurar con `write_text()` en Windows re-codifica a CRLF y produce un `restaurado=False` falso — se corrigió con `newline=""`.

**Archivos Nuevos**: `modules/asset_generation/whatsapp_setup_guide.py`, `tests/commercial_documents/test_fase_b_promesa_whatsapp.py` (12 funciones canónicas), `evidence/REFACTOR-WHATSAPP-ENTREGA-2026-09-18/FASE-B/` — `resultados-y-observaciones.md`, `CHECKPOINT-autorizaciones-pendientes.md`, `A4-decision.md`, `seleccion_pertinente.txt`, `tests_baseline_pre.txt`, `tests_baseline_post.txt`, `tests_postfull_regresion.txt`, `tests_a4_postfull.txt`, `quick_pre.txt`, `run_mutations.py`, `mutation_report.json`.

**Archivos Modificados**: producción — `modules/commercial_documents/pain_solution_mapper.py`, `modules/asset_generation/asset_catalog.py`, `conditional_generator.py`, `pain_ledger.py`, `whatsapp_conflict_guide.py`, `v4_asset_orchestrator.py`, `proposal_asset_alignment.py`, `modules/common/service_identity.py`, `modules/commercial_documents/v4_diagnostic_generator.py`, `scripts/validate_wiring.py`, `main.py` (solo comentario). Tests re-vinculados por cambio de forma, sin aflojar ninguna aserción: `test_proposal_alignment.py`, `test_proposal_dynamic.py`, `test_promised_assets_production.py`, `test_coherence_generated_assets.py`, `test_site_verification_propagation.py`, `test_validate_wiring.py`, `test_proposal_asset_matrix.py`, `test_fase_c_propuesta_dinamica.py`, y `tests/quality_gates/test_publication_gates.py` (A4: fixture re-anclado + aserción reforzada + test de caracterización de la deuda AC5). Más los cuatro documentos del plan.

**A4 (decisión de gate, misma sesión 2026-09-20)**: el rojo residual se resolvió con la opción **O5** (`evidence/…/FASE-B/A4-decision.md`, con cinco opciones evaluadas y sus costes medidos). `tests/quality_gates/test_publication_gates.py::test_get_blocking_issues` re-ancla su fixture de `no_whatsapp_visible` a `whatsapp_conflict` —el dolor de WhatsApp que sigue prometiendo un servicio contado, `boton_whatsapp`, con el que el gate bloquea por la vía real (`actionable_total=1`, `coverage_ratio=0.0`, `BLOCKED`)— y **refuerza** la aserción con `assert "proposal_asset_alignment" in blocking_names`. El punto ciego no se tapó: nace `test_deuda_ac5_ledger_solo_condicional_pasa_trivial`, que aserta el "PASS trivial" de hoy (`actionable_total == 0`, `coverage_ratio == 1.0`) y declara que **debe ponerse rojo cuando AC5 lo gobierne**. Ninguna línea del gate se tocó; dueños, umbrales, `BLOCKING_VERDICTS`, `GATE_BLOCKING_ENABLED` y `write/publish/suppress` intactos. Dueño de la deuda: **AC5, C-D** (se corrige el "D-E" registrado antes: E no posee AC5).

**Tests**: selección literal de 25 archivos: PRE **537 passed** → POST-A **539** (delta +2, dos casos nuevos de AC19a en un archivo de la selección) → POST-B **552** con la suite nueva. Superficie de matriz/gate tras A1: **83 passed**. Regresión completa tras B: **4 failed, 4.261 passed, 41 skipped, 4 xfailed** en 244 s — tres rojos preexistentes con dueño (`test_function_default_flags` flaky, `test_diagnostic_includes_geo_metrics`, `test_medido_contra_el_predecesor…`) y uno nuevo de causa medida, cerrado por A4. Regresión tras A4: **3 failed, 4.263 passed, 41 skipped, 4 xfailed** en 196,5 s — **los tres rojos son exactamente los de la evidencia de FASE-0; ninguno atribuible a B**. A4 medido con la misma selección extendida (25 + `test_publication_gates.py`): PRE sobre `473ed0f` en `git worktree --detach` = **1 failed / 619 passed / 1 skipped**; POST = **0 failed / 621 passed / 1 skipped**. Funciones canónicas **4.285 → 4.299 (+14) → 4.300 (+1)**. Quick **11/11** al PRE y re-medido al cerrar A4. **Contador v4complete 0/1**: ninguna corrida real. R2 **FUERA DE SERVICIO (R2.1)** — los autocuentos intermedios (~155/~165) se retiran por no haber instrumento. **Commit `473ed0f` ejecutado el 2026-09-20 con instrucción literal del operador** (42 archivos, +1.907/−266; los 7 checks del pre-commit pasaron sin saltar ninguno); **push a `origin/master` ejecutado el 2026-09-20 con instrucción literal del operador (`cf3ddc2..05d0cc6`; paridad 0/0 verificada con `git ls-remote`)**.
