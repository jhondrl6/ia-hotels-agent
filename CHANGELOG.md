...
**Tests**: selección literal de 12 archivos: PRE **189 passed / 1 skipped** y POST-B **189** con el código de la fase aplicado (**delta 0**); POST-A **210 passed / 1 skipped** (exit 0). La suite nueva contra `git archive HEAD` rompe **16** (13 por aserción, 3 por `ImportError` del helper nuevo, declarados). Regresión completa: **3 failed, 4.247 passed, 41 skipped, 4 xfailed** en 232 s — los tres rojos preexistentes con dueño, ninguno de esta fase. Funciones canónicas **4.264 → 4.285 (+21)**. Quick **11/11**. **Contador v4complete 0/1**: ninguna ejecución real. Sin commit ni push: autorización no solicitada; el trabajo queda como checkpoint en el árbol.
